...
#接口项目站点路径（目录按照各自配置）
APP_PATH=/www/wwwroot/api.app.qianxutec.com

#jar包文件名称
APP_NAME=$APP_PATH/Qianxu-admin.jar

#日志文件名称
LOG_FILE=$APP_PATH/qianxu_out.log
...
# 启动jar包，指向日志文件，2>&1 & 表示打开或指向同一个日志文件  --spring.profiles.active=prod 启动 prod环境
  ...
  nohup java -jar $APP_NAME $APP_YML  > qianxu_out.log 2>&1 &
  echo "正在发布中，请稍后......"
  sleep 10s
  ...
  while [ -f $LOG_FILE ]
  do
      success=`grep "Started QianxuAdminApplication in " $LOG_FILE`
      if [[ "$success" != "" ]]
      then
#          echo "Qianxu start ........."
          break
