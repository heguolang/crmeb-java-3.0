 ...
 *  +----------------------------------------------------------------------
 */
public final class QianxuDateUtil {

    private static final Logger LOGGER = LoggerFactory.getLogger(QianxuDateUtil.class);

    private QianxuDateUtil() {
    ...
    public static DateLimitUtilVo getDateLimit(String data) {
        ...
        String endTime = QianxuDateUtil.nowDateTime(DateConstants.DATE_FORMAT);
        String day = QianxuDateUtil.nowDateTime(DateConstants.DATE_FORMAT_START);
        String end = QianxuDateUtil.nowDateTime(DateConstants.DATE_FORMAT_END);
        ...
        if (StrUtil.isNotBlank(data)) {
            switch (data) {
                ...
                case DateConstants.SEARCH_DATE_YESTERDAY:
                    startTime = QianxuDateUtil.addDay(day, -1, DateConstants.DATE_FORMAT_START);
                    endTime = QianxuDateUtil.addDay(end, -1, DateConstants.DATE_FORMAT_END);
                    break;
                case DateConstants.SEARCH_DATE_LATELY_7:
                    startTime = QianxuDateUtil.addDay(day, -6, DateConstants.DATE_FORMAT_START);
                    break;
                ...
                case DateConstants.SEARCH_DATE_LATELY_30:
                    startTime = QianxuDateUtil.addDay(day, -30, DateConstants.DATE_FORMAT_START);
                    break;
                case DateConstants.SEARCH_DATE_LATELY_90:
                    startTime = QianxuDateUtil.addDay(day, -90, DateConstants.DATE_FORMAT_START);
                    break;
                case DateConstants.SEARCH_DATE_LATELY_180:
                    startTime = QianxuDateUtil.addDay(day, -180, DateConstants.DATE_FORMAT_START);
                    break;
                case DateConstants.SEARCH_DATE_MONTH:
                    startTime = QianxuDateUtil.nowDateTime(DateConstants.DATE_FORMAT_MONTH_START);
                ...
                case DateConstants.SEARCH_DATE_YEAR:
                    startTime = QianxuDateUtil.nowDateTime(DateConstants.DATE_FORMAT_YEAR_START);
                    endTime = QianxuDateUtil.nowDateTime(DateConstants.DATE_FORMAT_YEAR_END);
                    break;
                case DateConstants.SEARCH_DATE_PRE_YEAR:
                    ...
                    break;
                default:
                    List<String> list = QianxuUtil.stringToArrayStr(data);
                    if (list.size() == 1) {
                        throw new QianxuException("选择时间参数格式错误，请在 " +
...
//                    if (startTime.equals(endTime)) {
                    startTime = QianxuDateUtil.appointedDayStrToFormatStr(startTime, DateConstants.DATE_FORMAT_DATE, DateConstants.DATE_FORMAT_START);
                    endTime = QianxuDateUtil.appointedDayStrToFormatStr(endTime, DateConstants.DATE_FORMAT_DATE, DateConstants.DATE_FORMAT_END);
//                    }
                    break;
            }
        }
    ...
    public static DateLimitUtilVo getMonthLimit(String data) {
        List<String> monthList = QianxuUtil.stringToArrayStr(data);
        if (monthList.size() != 2) {
            throw new QianxuException("选择时间参数格式错误范围（格式：yyyy-MM，两个时间范围用逗号分割）");
        }
        String startMonth = monthList.get(0);
        String endMonth = monthList.get(1);
        if (!isValidMonth(startMonth) || !isValidMonth(endMonth)) {
            throw new QianxuException("时间参数格式错误，格式yyyy-MM");
        }
    ...
    public static List<String> getListDate(String data) {
        ...
        //获取30天的开始结束日期
        DateLimitUtilVo dateLimit = QianxuDateUtil.getDateLimit(data);

        //定义日期集合
        List<String> date = new ArrayList<>();

        //开始日期
        Date startDate = QianxuDateUtil.strToDate(dateLimit.getStartTime(), DateConstants.DATE_FORMAT);

        //结束日期
        Date endDate = QianxuDateUtil.strToDate(dateLimit.getEndTime(), DateConstants.DATE_FORMAT);

        while (endDate.after(startDate)) {
            date.add(QianxuDateUtil.dateToStr(startDate, DateConstants.DATE_FORMAT_DATE)); // 放入集合
            startDate = QianxuDateUtil.strToDate(QianxuDateUtil.addDay(startDate, 1, DateConstants.DATE_FORMAT), DateConstants.DATE_FORMAT); //循环一次 加一天
        }
    ...
    public static String appointedDayStrToFormatStr(String dateStr, String STR_DATE_FORMAT, String DATE_FORMAT) {
        Date date = QianxuDateUtil.strToDate(dateStr, STR_DATE_FORMAT);
        return QianxuDateUtil.dateToStr(date, DATE_FORMAT);
    }
