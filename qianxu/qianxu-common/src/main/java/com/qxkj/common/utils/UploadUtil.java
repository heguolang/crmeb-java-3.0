...
//    private static String modelPath = "/store";
    ...
    public static File createFile(String filePath) throws IOException {
        ...
        if (!file.isDirectory()) {
            //创建失败返回null
            if (!file.mkdirs()) {
                throw new QianxuException("文件目录创建失败...");
            }
        }
        // 判断这个文件是否存在，不存在就创建
        ...
        if (!file.exists()) {
            if (!file.createNewFile()) {
                throw new QianxuException("目标文件创建失败...");
            }
        }
        return file;
    }
    ...
    public static String fileName(String extName){
        return QianxuUtil.getUuid() + RandomUtil.randomString(10) + "." + extName;
    }
    ...
    public static String getWebPath() {
        // 文件分隔符转化为当前系统的格式
        return getModelPath() + QianxuDateUtil.nowDate(Constants.DATE_FORMAT_DATE).replace("-", "/") + "/";
