...
import com.qxkj.common.constants.RedisConstatns;
...
import com.qxkj.common.request.CartNumRequest;
import com.qxkj.common.request.CartRequest;
import com.qxkj.common.request.CartResetRequest;
import com.qxkj.common.request.PageParamRequest;
import com.qxkj.common.response.CartInfoResponse;
import com.qxkj.common.utils.RedisUtil;
import com.qxkj.service.dao.StoreCartDao;
    ...
    */
    ...
    public String saveCate(CartRequest storeCartRequest) {
        ...
        if (ObjectUtil.isNull(product) || product.getIsDel() || !product.getIsShow()) {
            throw new QianxuException("未找到对应商品");
        }
        ...
        if (CollUtil.isEmpty(attrValues)) {
            throw new QianxuException("未找到对应的商品SKU");
        }
        ...
        if (existCarts.size() > 0) { // 购物车添加数量
            ...
            if (!updateResult) throw new QianxuException("添加购物车失败");
            ...
            if (dao.insert(storeCart) <= 0) throw new QianxuException("添加购物车失败");
            // 商品加购量统计(每日/商城)
            redisUtil.incrAndCreate(RedisConstatns.PRO_ADD_CART_KEY + todayStr);
            // 商品加购量统计(每日/个体)
            redisUtil.incrAndCreate(StrUtil.format(RedisConstatns.PRO_PRO_ADD_CART_KEY, todayStr, storeCartRequest.getProductId()));
    ...
    public Boolean resetCart(CartResetRequest resetRequest) {
        ...
        if (ObjectUtil.isNull(storeCart)) throw new QianxuException("购物车不存在");
        if (ObjectUtil.isNull(resetRequest.getNum()) || resetRequest.getNum() <= 0 || resetRequest.getNum() >= 999)
            throw new QianxuException("数量不合法");
        ...
        if (!updateResult) throw new QianxuException("重选添加购物车失败");
    ...
    public Boolean updateCartNum(Integer id, Integer number) {
        if (ObjectUtil.isNull(number)) throw new QianxuException("商品数量不合法");
        if (number <=0 || number > 99) throw new QianxuException("商品数量不能小于1大于99");
        StoreCart storeCart = getById(id);
        if (ObjectUtil.isNull(storeCart)) throw new QianxuException("当前购物车不存在");
