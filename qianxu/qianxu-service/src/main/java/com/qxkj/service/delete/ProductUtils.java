...
//        JSONObject tbJsonData = new JSONObject(JSONExample.tbJson); // just Test
        ...
        if (null == data) throw new QianxuException("复制商品失败--返回数据格式错误--未找到data");
        JSONObject item = data.getJSONObject("item");
        if (null == item) throw new QianxuException("复制商品失败--返回数据格式错误--未找到item");
...
//        if (null == props) throw new QianxuException("复制商品失败--返回数据格式错误--未找到props");
...
//        String token = systemConfigService.getValueByKey("importProductToken");
        ...
        if (StringUtils.isBlank(token)) {
            throw new QianxuException("请配置复制产品平台的Token -- www.99api.com");
        }
        if (StringUtils.isBlank(baseUrl)) {
            throw new QianxuException("请配置复制产品平台的Url-- www.99api.com");
        }
//        rightUrl = "?apikey="+systemConfigService.getValueByKey("importProductToken")+rightEndUrl;
    ...
    public HashMap<Integer, ProductActivityItemResponse> getActivityByProduct(Integer productId, String activity) {
        ...
        List<Integer> activitys = QianxuUtil.stringToArrayInt(activity);
        for (Integer code : activitys) {
            if (code == 1) { // 查找秒杀信息
                ...
                if (null != currentSecKills && currentSecKills.size() > 0) {
                    ...
                    int secKillEndSecondTimestamp =
                            QianxuDateUtil.getSecondTimestamp(QianxuDateUtil.nowDateTime("yyyy-MM-dd " + secKillManager.getEndTime() + ":00:00"));
            ...
            if (code == 2) { // 查找砍价信息
                ...
                if (CollUtil.isNotEmpty(currentBargains)) {
                    ...
                    bargainResponse.setTime(QianxuDateUtil.getSecondTimestamp(currentBargains.get(0).getStopTime()));
            ...
            if (code == 3) { // 查找拼团信息
                ...
                if (CollUtil.isNotEmpty(currentCombinations)) {
                    ...
                    bargainResponse.setTime(QianxuDateUtil.getSecondTimestamp(currentCombinations.get(0).getStopTime()));
