...
import com.qxkj.service.dao.StoreProductReplyDao;
    ...
    */
    ...
    public PageInfo<StoreProductReplyResponse> getList(StoreProductReplySearchRequest request) {
        ...
        for (StoreProductReply productReply : dataList) {
            ...
            productReplyResponse.setPics(QianxuUtil.stringToArrayStr(productReply.getPics()));
    ...
    public Boolean create(StoreProductReplyAddRequest request) {
        ...
        if (ObjectUtil.isNull(storeOrder) || !storeOrder.getUid().equals(user.getUid())) {
            throw new QianxuException("该订单不存在");
        }
        ...
        if (!execute) {
            throw new QianxuException("评价订单失败");
        }
        return execute;
    }
    ...
    public boolean virtualCreate(StoreProductReplyAddRequest request) {
        ...
        storeProductReply.setUnique(QianxuUtil.randomCount(11111,9999)+"");
    ...
    public ProductDetailReplyResponse getH5ProductReply(Integer proId) {
        ...
        // 评价图
        productReplyResponse.setPics(QianxuUtil.stringToArrayStr(storeProductReply.getPics()));
        // 昵称
    ...
    public Boolean comment(StoreProductReplyCommentRequest request) {
        ...
        lup.set(StoreProductReply::getMerchantReplyTime, QianxuDateUtil.getNowTime());
    ...
    private Integer checkIsReply(StoreProductReply storeProductReply) {
        ...
        if (null == orderInfoVoList || orderInfoVoList.size() < 1) {
            throw new QianxuException("没有找到商品信息");
        }
...
//            Integer productId = orderInfoVo.getInfo().getInteger("product_id");
        ...
        if (replyCount > 0) {
            throw new QianxuException("该商品已评价");
        }
