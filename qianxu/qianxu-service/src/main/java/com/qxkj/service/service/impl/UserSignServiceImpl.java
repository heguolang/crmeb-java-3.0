...
import com.qxkj.common.constants.IntegralRecordConstants;
...
import com.qxkj.service.dao.UserSignDao;
...
public class UserSignServiceImpl extends ServiceImpl<UserSignDao, UserSign> implements UserSignService {
    ...
    public SystemGroupDataSignConfigVo sign() {
        ...
        if (ObjectUtil.isNull(lastUserSign)) {
            ...
            // 判断是否重复签到
            String lastDate = QianxuDateUtil.dateToStr(lastUserSign.getCreateDay(), Constants.DATE_FORMAT_DATE);
            String nowDate = QianxuDateUtil.nowDate(Constants.DATE_FORMAT_DATE);
            //对比今天数据
            if (QianxuDateUtil.compareDate(lastDate, nowDate, Constants.DATE_FORMAT_DATE) == 0) {
                throw new QianxuException("今日已签到。不可重复签到");
            }
            String nextDate = QianxuDateUtil.addDay(lastUserSign.getCreateDay(), 1, Constants.DATE_FORMAT_DATE);
            int compareDate = QianxuDateUtil.compareDate(nextDate, nowDate, Constants.DATE_FORMAT_DATE);
            if (compareDate != 0) {
                //不相等，所以不是连续签到,重置签到次数
                user.setSignNum(0);
            }
        ...
        if (CollUtil.isEmpty(config)) {
            throw new QianxuException("签到配置不存在，请在管理端配置签到数据");
        }
        ...
        if (ObjectUtil.isNull(configVo)) {
            throw new QianxuException("请先配置签到天数！");
        }
        ...
        userSign.setCreateDay(QianxuDateUtil.strToDate(QianxuDateUtil.nowDate(Constants.DATE_FORMAT_DATE), Constants.DATE_FORMAT_DATE));
        ...
        if (!execute) {
            throw new QianxuException("修改用户签到信息失败!");
        }
    ...
    public List<UserSignMonthVo> getListGroupMonth(PageParamRequest pageParamRequest) {
        ...
        for (UserSign userSign : userSignList) {
            String date = QianxuDateUtil.dateToStr(userSign.getCreateDay(), Constants.DATE_FORMAT_MONTH);
    ...
    private Boolean checkDaySign(Integer userId) {
        List<UserSign> userSignList = getInfoByDay(userId, QianxuDateUtil.nowDate(Constants.DATE_FORMAT_DATE));
    ...
    private Boolean checkYesterdaySign(Integer userId) {
        String day = QianxuDateUtil.nowDate(Constants.DATE_FORMAT_DATE);
        String yesterday = QianxuDateUtil.addDay(day, -1, Constants.DATE_FORMAT_DATE);
