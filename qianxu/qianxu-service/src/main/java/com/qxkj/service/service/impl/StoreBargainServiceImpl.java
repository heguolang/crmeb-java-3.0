...
import com.qxkj.common.constants.BargainConstants;
...
import com.qxkj.service.dao.StoreBargainDao;
    ...
    */
    ...
    public PageInfo<StoreBargainResponse> getList(StoreBargainSearchRequest request, PageParamRequest pageParamRequest) {
        ...
        for (StoreBargain storeBargain : storeBargainList) {
            ...
            storeBargainResponse.setStartTime(QianxuDateUtil.timestamp2DateStr(storeBargain.getStartTime(), Constants.DATE_FORMAT_DATE));
            storeBargainResponse.setStopTime(QianxuDateUtil.timestamp2DateStr(storeBargain.getStopTime(), Constants.DATE_FORMAT_DATE));
            storeBargainResponse.setAddTime(QianxuDateUtil.timestamp2DateStr(storeBargain.getAddTime(), Constants.DATE_FORMAT));
    ...
    public boolean saveBargain(StoreBargainRequest request) {
        ...
        if (tempPrice.compareTo(multiply) < 0) {
            ...
            throw new QianxuException("必须保证每个人最少砍1分钱");
        }
        ...
        // 活动开始结束时间
        bargain.setStartTime(QianxuDateUtil.dateStr2Timestamp(request.getStartTime(), Constants.DATE_TIME_TYPE_BEGIN));
        bargain.setStopTime(QianxuDateUtil.dateStr2Timestamp(request.getStopTime(), Constants.DATE_TIME_TYPE_END));
    ...
    public boolean updateBargain(StoreBargainRequest request) {
        ...
        if (saveCount <= 0) {
            throw new QianxuException("编辑砍价商品失败");
        }
    ...
    public StoreProductInfoResponse getAdminDetail(Integer bargainId) {
        ...
        if (ObjectUtil.isNull(storeBargain) || storeBargain.getIsDel()) {
            throw new QianxuException("未找到对应砍价商品信息");
        }
    ...
    public BargainDetailH5Response getH5Detail(Integer id) {
        ...
        if (!storeBargain.getStatus()) {
            throw new QianxuException("砍价商品已下架");
        }
        ...
        if (StrUtil.isNotBlank(storeProduct.getGuaranteeIds())) {
            detailH5Response.setGuaranteeList(guaranteeService.findByIdList(QianxuUtil.stringToArray(storeProduct.getGuaranteeIds())));
        }
        ...
        if (CollUtil.isEmpty(storeProductAttrValuesBargain)) {
            throw new QianxuException("砍价商品规格属性值未找到");
        }
    ...
    public MyRecord start(BargainFrontRequest request) {
        ...
        if (ObjectUtil.isNull(storeBargain) || storeBargain.getIsDel()) {
            throw new QianxuException("对应的砍价商品不存在");
        }
        if (!storeBargain.getStatus()) {
            throw new QianxuException("砍价商品已下架");
        }
        if (storeBargain.getQuota() <= 0 || storeBargain.getStock() <= 0) {
            throw new QianxuException("砍价商品已售罄");
        }
        ...
        if (CollUtil.isNotEmpty(historyList)) {
            ...
            if (CollUtil.isNotEmpty(collect)) {
                throw new QianxuException("请先完成当前砍价活动");
            }
            // 判断是否达到参与砍价活动上限
            if (historyList.size() >= storeBargain.getNum()) {
                throw new QianxuException("您已达到当前砍价活动上限");
            }
        }
        ...
        if (!save) {
            throw new QianxuException("参与砍价失败");
        }
    ...
    public void stopAfterChange() {
        ...
        if (!b) {
            ...
            throw new QianxuException("砍价活动结束后更新用户状态失败");
        }
    }
    ...
    public StoreBargain getByIdException(Integer id) {
        ...
        if (ObjectUtil.isNull(storeBargain)) throw new QianxuException("砍价商品不存在或未开启");
        return storeBargain;
    }
    ...
    public Boolean operationStock(Integer id, Integer num, String type) {
        ...
        if (!update) {
            throw new QianxuException("更新砍价商品库存失败,商品id = " + id);
        }
        return update;
    }
