...
import com.qxkj.common.response.StoreOrderDetailResponse;
import com.qxkj.common.response.StoreProductResponse;
import com.github.pagehelper.PageInfo;
import com.qxkj.common.utils.QianxuUtil;
import com.qxkj.common.utils.QianxuDateUtil;
import com.qxkj.common.utils.ExportUtil;
import com.qxkj.common.response.StoreBargainResponse;
import com.qxkj.common.response.StoreCombinationResponse;
import com.qxkj.common.vo.BargainProductExcelVo;
import com.qxkj.common.vo.CombinationProductExcelVo;
import com.qxkj.common.vo.OrderExcelVo;
import com.qxkj.common.vo.ProductExcelVo;
...
public class ExcelServiceImpl implements ExcelService {
    ...
    public String exportBargainProduct(StoreBargainSearchRequest request) {
        ...
        if (CollUtil.isEmpty(page.getList())) throw new QianxuException("没有可导出的数据!");
        ...
        // 上传设置
        ExportUtil.setUpload(qianxuConfig.getImagePath(), Constants.UPLOAD_MODEL_PATH_EXCEL, UploadConstants.DOWNLOAD_FILE_KEYWORD);

        // 文件名
        String fileName = "砍价".concat(QianxuDateUtil.nowDateTime(Constants.DATE_TIME_FORMAT_NUM)).concat(QianxuUtil.randomCount(111111111, 999999999).toString()).concat(".xlsx");
...
//        aliasMap.put("info", "砍价活动简介");
    ...
    public String exportCombinationProduct(StoreCombinationSearchRequest request) {
        ...
        List<CombinationProductExcelVo> voList = list.stream().map(temp -> {
            ...
            vo.setStopTime(QianxuDateUtil.timestamp2DateStr(temp.getStopTime(), Constants.DATE_FORMAT_DATE));
            return vo;
        }).collect(Collectors.toList());
        ...
        // 文件名
        String fileName = "拼团".concat(QianxuDateUtil.nowDateTime(Constants.DATE_TIME_FORMAT_NUM)).concat(QianxuUtil.randomCount(111111111, 999999999).toString()).concat(".xlsx");
    ...
    public String exportProduct(StoreProductSearchRequest request) {
        ...
        if(list.size() < 1){
            throw new QianxuException("没有可导出的数据！");
        }
        ...
        if(cateIdListStr.size() > 0){
            ...
            List<Integer> cateIdList = QianxuUtil.stringToArray(join);
...
//            vo.setStoreInfo(product.getStoreInfo());
            vo.setCateName(QianxuUtil.getValueByIndex(categoryNameList, product.getCateId()));
        ...
        // 文件名
        String fileName = "商品导出_".concat(QianxuDateUtil.nowDateTime(Constants.DATE_TIME_FORMAT_NUM)).concat(QianxuUtil.randomCount(111111111, 999999999).toString()).concat(".xlsx");
...
//        aliasMap.put("storeInfo", "商品简介");
    ...
    public String exportOrder(StoreOrderSearchRequest request) {
        ...
        for (StoreOrderDetailResponse order: list ) {
            ...
            vo.setCreateTime(QianxuDateUtil.dateToStr(order.getCreateTime(), "yyyy-MM-dd HH:mm:ss"));
        ...
        // 文件名
        String fileName = "订单导出_".concat(QianxuDateUtil.nowDateTime(Constants.DATE_TIME_FORMAT_NUM)).concat(QianxuUtil.randomCount(111111111, 999999999).toString()).concat(".xlsx");
