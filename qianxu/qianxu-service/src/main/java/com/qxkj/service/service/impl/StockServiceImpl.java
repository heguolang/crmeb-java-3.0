...
import com.qxkj.common.model.stock.StockChangeLog;
import com.qxkj.common.model.stock.StockLevel;
import com.qxkj.common.model.stock.StockLadder;
...
import com.qxkj.common.model.stock.StockPrice;
import com.qxkj.common.model.stock.StockPriceSku;
import com.qxkj.common.model.product.StoreProductAttrValue;
import com.qxkj.common.model.stock.StockProductRel;
...
import com.qxkj.service.dao.StockAgentDao;
import com.qxkj.service.dao.StockLevelDao;
import com.qxkj.service.dao.StockLogDao;
import com.qxkj.service.dao.StockPriceDao;
...
public class StockServiceImpl implements StockService {
    ...
    @Autowired
    private com.qxkj.service.dao.StockLadderDao stockLadderDao;

    @Autowired
    private com.qxkj.service.dao.StockPriceSkuDao stockPriceSkuDao;

    @Autowired
    private com.qxkj.service.dao.StoreProductAttrValueDao storeProductAttrValueDao;
    ...
    @Resource
    private com.qxkj.service.dao.StockOrderDao stockOrderDao;

    @Resource
    private com.qxkj.service.dao.StockOrderProductDao stockOrderProductDao;

    @Resource
    private com.qxkj.service.dao.StockChangeLogDao stockChangeLogDao;

    @Resource
    private com.qxkj.service.dao.StockProductRelDao stockProductRelDao;

    @Resource
    private com.qxkj.service.dao.StockVirtualStockDao stockVirtualStockDao;

    @Resource
    private com.qxkj.service.dao.StockExchangeDao stockExchangeDao;
    ...
    public Boolean deleteLevel(Integer id) {
        ...
        if (used != null && used > 0) {
            throw new QianxuException("该层级下存在订货代理，无法删除");
        }
    ...
    public Boolean saveAgent(StockAgentRequest request) {
        ...
        if (level == null || level.getIsDel() == 1) {
            throw new QianxuException("层级不存在");
        }
        ...
        if (request.getParentId() != null && request.getParentId() > 0) {
            ...
            if (parent == null || parent.getIsDel() == 1) {
                throw new QianxuException("上级代理不存在");
            }
        ...
        if (request.getId() != null && request.getId() > 0) {
            ...
            if (exist.getParentId() != null && exist.getParentId() > 0) {
                ...
                if (subIds.contains(exist.getParentId())) {
                    throw new QianxuException("上级不能是自己团队的下级代理");
                }
            }
        ...
        if (count != null && count > 0) {
            throw new QianxuException("该用户已是订货代理");
        }
    ...
    public Boolean deleteAgent(Integer id) {
        ...
        if (subCount != null && subCount > 0) {
            throw new QianxuException("该代理存在下级，请先处理下级代理");
        }
    ...
    public void saveProductStockType(Integer productId, Boolean supportVirtual, Boolean supportPhysical) {
        ...
        if (rel == null) {
            throw new QianxuException("该商品尚未加入订货模块");
        }
    ...
    public Boolean addProducts(List<Integer> productIds) {
        if (productIds == null || productIds.isEmpty()) {
            throw new QianxuException("请选择要添加的商品");
        }
        return transactionTemplate.execute(status -> {
            ...
            for (Integer pid : productIds) {
                ...
                if (product == null || product.getIsDel()) {
                    throw new QianxuException("商品不存在（ID:" + pid + "）");
                }
    ...
    private List<HashMap<String, Object>> enrichAdjustRows(List<HashMap<String, Object>> rows) {
        ...
        if (!productIds.isEmpty()) {
            for (com.qxkj.common.model.product.StoreProduct p : storeProductService.listByIds(productIds)) {
    ...
    public Boolean adjustStock(StockRequests.StockAdjustRequest request, Integer adminId) {
        if (request.getChangeNum() == 0) {
            throw new QianxuException("变动数量不能为0");
        }
        return transactionTemplate.execute(status -> {
            StoreProduct product = storeProductService.getById(request.getProductId());
            if (product == null || product.getIsDel()) {
            ...
            if (after < 0) {
                throw new QianxuException("库存不足，当前库存：" + product.getStock());
            }
    ...
    private void fillLogs(List<StockLog> list) {
        ...
        if (!linkNos.isEmpty()) {
            for (com.qxkj.common.model.stock.StockOrder o : stockOrderDao.selectList(
                    new LambdaQueryWrapper<com.qxkj.common.model.stock.StockOrder>()
                            .in(com.qxkj.common.model.stock.StockOrder::getOrderNo, linkNos))) {
                linkUidMap.put(o.getOrderNo(), o.getUid());
            }
            for (com.qxkj.common.model.stock.StockExchange e : stockExchangeDao.selectList(
                    new LambdaQueryWrapper<com.qxkj.common.model.stock.StockExchange>()
                            .in(com.qxkj.common.model.stock.StockExchange::getExchangeNo, linkNos))) {
    ...
    public void adjustAgentVirtualStock(StockRequests.StockAgentAdjustRequest request) {
        ...
        if (request.getNum() == null || request.getNum() == 0) {
            throw new QianxuException("调整数量不能为0");
        }
        StoreProduct product = storeProductService.getById(request.getProductId());
        if (product == null || product.getIsDel()) {
            throw new QianxuException("商品不存在");
        ...
        transactionTemplate.executeWithoutResult(status -> {
            ...
            if (vs == null) {
                if (request.getNum() < 0) {
                    throw new QianxuException("该订货商无此虚拟库存记录，无法扣减");
                }
                ...
                if (remain < 0 || total < 0) {
                    throw new QianxuException("扣减数量超出该订货商虚拟库存（当前可提：" + vs.getRemainNum() + "）");
                }
    ...
    private StockAgent resolveAgent(Integer agentId, Integer uid) {
        ...
        if (agent == null) {
            throw new QianxuException("订货商不存在");
        }
        return agent;
    }
    ...
    public Boolean createSubAgent(Integer uid, StockAgentCreateRequest request) {
        ...
        if (parentLevel == null || level.getSort() < parentLevel.getSort()) {
            throw new QianxuException("下级代理层级不得高于自己（同级平推或更低均可）");
        }
        // 按手机号找已注册用户
        User user = userService.lambdaQuery().eq(User::getPhone, request.getPhone().trim()).one();
        if (user == null) {
            throw new QianxuException("该手机号尚未注册，请让对方先注册会员");
        }
        if (user.getUid().equals(uid)) {
            throw new QianxuException("不能把自己加为下级订货商");
        }
        Integer count = stockAgentDao.selectCount(new LambdaQueryWrapper<StockAgent>()
                .eq(StockAgent::getUid, user.getUid()).eq(StockAgent::getIsDel, 0));
        if (count != null && count > 0) {
            throw new QianxuException("该用户已是订货商，或已有待确认的邀请");
        }
        StockAgent agent = new StockAgent();
    ...
    public Boolean agreeSubAgent(Integer uid, boolean agree) {
        ...
        if (agent == null || agent.getStatus() == null || agent.getStatus() != StockAgent.STATUS_PENDING) {
            throw new QianxuException("没有待确认的订货商邀请");
        }
        boolean ok;
        if (agree) {
            LambdaUpdateWrapper<StockAgent> luw = new LambdaUpdateWrapper<>();
    ...
    public BigDecimal getProductPrice(StockAgent agent, Integer productId, String skuKey) {
        ...
        StoreProductAttrValue av = storeProductAttrValueDao.selectOne(new LambdaQueryWrapper<StoreProductAttrValue>()
                ...
                .last(" limit 1"));
        if (av == null) {
            throw new QianxuException("商品规格不存在或已删除");
        }
        if (agent == null) {
    ...
    public void deductStockBySku(Integer productId, String skuKey, Integer num, Integer type, String linkNo, String mark) {
        ...
        if (updated == 0) {
            throw new QianxuException("该规格云仓库存不足，当前库存：" + getSkuStock(productId, sku));
        }
    ...
    private BigDecimal sumPaidOrderAmount(List<Integer> agentIds) {
        ...
        List<com.qxkj.common.model.stock.StockOrder> orders = stockOrderDao.selectList(
                new LambdaQueryWrapper<com.qxkj.common.model.stock.StockOrder>()
                        .in(com.qxkj.common.model.stock.StockOrder::getAgentId, agentIds)
                        .eq(com.qxkj.common.model.stock.StockOrder::getPayStatus, 1)
                        .eq(com.qxkj.common.model.stock.StockOrder::getIsDel, 0)
                        // 已驳回(-1)/已取消(-2)的订货单不计入升级业绩
                        .notIn(com.qxkj.common.model.stock.StockOrder::getStatus,
                                com.qxkj.common.model.stock.StockOrder.STATUS_REJECT, com.qxkj.common.model.stock.StockOrder.STATUS_CANCEL));
        BigDecimal total = BigDecimal.ZERO;
        for (com.qxkj.common.model.stock.StockOrder order : orders) {
    ...
    private Set<Integer> getBoughtProductIds(Integer agentId) {
        ...
        List<com.qxkj.common.model.stock.StockOrder> orders = stockOrderDao.selectList(
                new LambdaQueryWrapper<com.qxkj.common.model.stock.StockOrder>()
                        .eq(com.qxkj.common.model.stock.StockOrder::getAgentId, agentId)
                        .eq(com.qxkj.common.model.stock.StockOrder::getPayStatus, 1)
                        .eq(com.qxkj.common.model.stock.StockOrder::getIsDel, 0)
        ...
        for (com.qxkj.common.model.stock.StockOrder o : orders) {
            orderIds.add(o.getId());
        }
        List<com.qxkj.common.model.stock.StockOrderProduct> products = stockOrderProductDao.selectList(
                new LambdaQueryWrapper<com.qxkj.common.model.stock.StockOrderProduct>()
                        .in(com.qxkj.common.model.stock.StockOrderProduct::getOrderId, orderIds));
        for (com.qxkj.common.model.stock.StockOrderProduct p : products) {
