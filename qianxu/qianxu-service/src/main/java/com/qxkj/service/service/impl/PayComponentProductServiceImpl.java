...
import com.qxkj.common.model.wechat.video.*;
...
import com.qxkj.common.request.StoreProductAttrValueAddRequest;
import com.qxkj.common.response.PayComponentProductResponse;
import com.qxkj.common.response.ProductDetailResponse;
import com.qxkj.common.response.StoreProductAttrValueResponse;
...
import com.qxkj.service.dao.PayComponentProductDao;
...
//        });
//        if (!execute) {
//            throw new QianxuException("第一步，本地保存商品时错误");
//        }
//
//        // 第二步，组装商品，发给组件
...
//        });
//        if (!execute1) {
//            throw new QianxuException("第三步，本地保存商品时错误");
//        }
//        return execute1;
//    }
    ...
    public Boolean delete(Integer proId) {
        ...
        if (!isDel) {
            throw new QianxuException("自定义交易组件删除商品失败");
        }
        product.setIsDel(true);
        Boolean execute = transactionTemplate.execute(e -> {
    ...
    public Boolean update(PayComponentProductAddRequest addRequest) {
        ...
        if (ObjectUtil.isNull(draftProduct) || draftProduct.getIsDel()) {
            throw new QianxuException("草稿商品不存在");
        }
        if (draftProduct.getEditStatus().equals(2)) {
            ...
            if (!delAudit) {
                throw new QianxuException("商品审核中，撤回审核失败!");
            }
        }
        ...
        if (!skuSave) {
            throw new QianxuException("第一步，本地保存商品sku时错误");
        }
        // 第二步，组装商品，发给组件
        ShopSpuAddVo shopSpuAddVo = assembleShopSpuVo(product, skuList, productInfo);
        ...
        if (!save) {
            throw new QianxuException("第三步，本地保存草稿商品时错误");
        }
        return save;
    }
    ...
    public Boolean listing(Integer proId) {
        ...
        if (!product.getStatus().equals(11)) {
            throw new QianxuException("商品只有在自主下架状态，才能上架");
        }
        ShopSpuCommonVo shopSpuCommonVo = new ShopSpuCommonVo();
        shopSpuCommonVo.setOutProductId(proId.toString());
        ...
        if (!shopSpuVo.getStatus().equals(11)) {
            throw new QianxuException("组件中商品不属于自主下架状态");
        }
        Boolean isListing = wechatVideoSpuService.shopSpuListing(shopSpuCommonVo);
        if (!isListing) {
            throw new QianxuException("组件中商品上架失败");
        }
        product.setStatus(5);
        product.setUpdateTime(DateUtil.date().toString());
    ...
    public Boolean delisting(Integer proId) {
        ...
        if (!product.getStatus().equals(0) && !product.getStatus().equals(5)) {
            throw new QianxuException("商品只有在初始值/上架状态，才能下架");
        }
        ShopSpuCommonVo shopSpuCommonVo = new ShopSpuCommonVo();
        shopSpuCommonVo.setOutProductId(proId.toString());
        shopSpuCommonVo.setNeedEditSpu(0);
        ...
        if (!isDelisting) {
            throw new QianxuException("组件中商品下架失败");
        }
    ...
    public ProductDetailResponse getH5Detail(Integer id) {
        ...
        if (!product.getStatus().equals(5)) {
            throw new QianxuException("商品未上架");
        }
    ...
    public Boolean operationStock(Integer productId, Integer num, String operationType) {
        ...
        if (!update) {
            throw new QianxuException("更新组件商品库存失败，商品Id = " + productId);
        }
        return update;
    }
    ...
    private ShopSpuAddVo assembleShopSpuVo(PayComponentProduct product, List<PayComponentProductSku> skuList, PayComponentProductInfo productInfo) {
        ...
        shopSpuAddVo.setHeadImg(QianxuUtil.stringToArrayStr(product.getHeadImg()));
        if (StrUtil.isNotBlank(product.getQualificationPics())) {
            shopSpuAddVo.setQualificationPics(QianxuUtil.stringToArrayStr(product.getQualificationPics()));
        }
