...
public class StoreOrderServiceImpl extends ServiceImpl<StoreOrderDao, StoreOrder> implements StoreOrderService {
    ...
    */
    @Override
    public CommonPage<StoreOrderDetailResponse> getAdminList(StoreOrderSearchRequest request) {
        ...
        List<StoreOrder> orderList = dao.getAdminOrderList(map);
    ...
    public PageInfo<StoreOrder> getUserOrderList_2_3(String keywords,
        ...
        // 新需求：H5订单列表，增加关键字搜索功能--> 所以注掉以下代码
//        LambdaQueryWrapper<StoreOrder> lqw = new LambdaQueryWrapper<>();
//        if(StrUtil.isNotBlank(keywords)){
//            orderUtils.statusApiByWhere(lqw, status);
//            lqw.eq(StoreOrder::getUid, uid);
//            lqw.orderByDesc(StoreOrder::getId);
//            List<StoreOrder> storeOrders = dao.selectList(lqw);
//            return CommonPage.copyPageInfo(page, storeOrders);
//        }
    ...
    public boolean refund(StoreOrderRefundRequest request) {
        ...
        if (storeOrder.getPayType().equals(Constants.PAY_TYPE_WE_CHAT) && request.getAmount().compareTo(BigDecimal.ZERO) > 0) {
            try {
                ...
                e.printStackTrace();
                throw new QianxuException("微信申请退款失败！");
            }
        }
        if (storeOrder.getPayType().equals(Constants.PAY_TYPE_ALI_PAY) && request.getAmount().compareTo(BigDecimal.ZERO) > 0) {
            try {
                ...
                e.printStackTrace();
                throw new QianxuException("支付宝申请退款失败！");
            }
        }
        ...
        if (!execute) {
            ...
            throw new QianxuException("订单更新失败");
        }
...
//    }
    ...
    public StoreOrderInfoResponse info(String orderNo) {
        ...
        if (storeOrder.getIsSystemDel()) {
            throw new QianxuException("未找到对应订单信息");
        }
        ...
        if (qianxuConfig.getDemoSite()) {
            storeOrderInfoResponse.setUserPhone(QianxuUtil.maskMobile(storeOrderInfoResponse.getUserPhone()));
            storeOrderInfoResponse.setPhone(QianxuUtil.maskMobile(storeOrderInfoResponse.getPhone()));
    ...
    public String send(StoreOrderSendRequest request) {
        ...
        //订单信息
        StoreOrder storeOrder = getInfoException(request.getOrderNo());
        if (storeOrder.getIsDel()) throw new QianxuException("订单已删除,不能发货!");
        if (storeOrder.getStatus() > 0) throw new QianxuException("订单已发货请勿重复操作!");
        if (ObjectUtil.isNotNull(storeOrder.getCombinationId()) && storeOrder.getCombinationId() > 0) {
            StorePink storePink = storePinkService.getById(storeOrder.getPinkId());
            if (storePink.getStatus() != 2) throw new QianxuException("当前订单正在拼团中不能发货！");
        }
        ...
        switch (request.getDeliveryType()) {
            ...
            case "fictitious":// 虚拟
                ...
                break;
            default:
                throw new QianxuException("类型错误");
        }
    ...
    public Boolean refundRefuse(String orderNo, String reason) {
        if (StrUtil.isBlank(reason)) {
            throw new QianxuException("请填写拒绝退款原因");
        }
        StoreOrder storeOrder = getInfoException(orderNo);
    ...
    public StoreOrderStatisticsResponse orderStatisticsByTime(String dateLimit, Integer type) {
        ...
        DateLimitUtilVo dateRange = QianxuDateUtil.getDateLimit(dateLimit);
        String dateStartD = dateRange.getStartTime();
        String dateEndD = dateRange.getEndTime();
        int days = QianxuDateUtil.daysBetween(
                QianxuDateUtil.strToDate(dateStartD,Constants.DATE_FORMAT_DATE),
                QianxuDateUtil.strToDate(dateEndD,Constants.DATE_FORMAT_DATE)
        );
        // 同时间区间的上一个时间起点
        String perDateStart = QianxuDateUtil.addDay(
                QianxuDateUtil.strToDate(dateStartD,Constants.DATE_FORMAT_DATE), -days, Constants.DATE_FORMAT_START);
        // 当前时间区间
        String dateStart = QianxuDateUtil.addDay(
                QianxuDateUtil.strToDate(dateStartD,Constants.DATE_FORMAT_DATE),0,Constants.DATE_FORMAT_START);
        String dateEnd = QianxuDateUtil.addDay(
                QianxuDateUtil.strToDate(dateEndD,Constants.DATE_FORMAT_DATE),0,Constants.DATE_FORMAT_END);
    ...
    public Boolean updatePaid(String orderNo) {
        ...
        lqw.set(StoreOrder::getPayTime, QianxuDateUtil.nowDateTime());
    ...
    public Boolean updatePrice(StoreOrderUpdatePriceRequest request) {
        ...
        if (existOrder.getPaid()) {
            throw new QianxuException(StrUtil.format("订单号为 {} 的订单已支付", existOrder.getOrderId()));
        }
        if (existOrder.getIsAlterPrice()) {
            throw new QianxuException("系统只支持一次改价");
        }
        // 修改价格和原来价格相同
        if (existOrder.getPayPrice().compareTo(request.getPayPrice()) ==0) {
            throw new QianxuException(StrUtil.format("修改价格不能和原支付价格相同 原价 {} 修改价 {}", existOrder.getPayPrice(), request.getPayPrice()));
        }
        ...
        if (!execute) {
            throw new QianxuException(Constants.RESULT_ORDER_EDIT_PRICE_SUCCESS
     ...
     */
    @Override
    public StoreOrderCountItemResponse getOrderStatusNum(StoreOrderTabsNumRequest request) {
    ...
    public Boolean delete(String orderNo) {
        ...
        if (!storeOrder.getIsDel()) {
            throw new QianxuException("您选择的的订单存在用户未删除的订单，无法删除用户未删除的订单！");
        }
        if (storeOrder.getIsSystemDel()) {
            throw new QianxuException("此订单已经被删除了!");
        }
    ...
    public Boolean videoSend(VideoOrderSendRequest request) {
        ...
        if (ObjectUtil.isNull(componentOrder)) {
            throw new QianxuException("为找到对应视频订单");
        }
        if (!componentOrder.getStatus().equals(20)) {
            throw new QianxuException("视频订单不处于可发货状态");
        }
        ...
        if (!deliverySend) {
            throw new QianxuException("发货失败");
        }
        ...
        if (!execute) throw new QianxuException("快递发货失败！");
...
//        wrapper.select("ifnull(if(sum(pay_price) = 0.00, 0, sum(pay_price)), 0) as pay_price");
    ...
    public Boolean updateTrackingNumber(StoreOrderSendRequest request) {
        StoreOrder storeOrder = getInfoException(request.getOrderNo());
        if (storeOrder.getIsDel()) throw new QianxuException("订单已删除,不能修改运单号!");
        if (storeOrder.getStatus() != 1) throw new QianxuException("待收货订单才能修改运单号");
    ...
    private StoreOrder getInfoException(String orderNo) {
        ...
        if (ObjectUtil.isNull(storeOrder)) {
            throw new QianxuException("没有找到订单信息");
        }
        return storeOrder;
    }
...
//        temMap.put("thing7", "您的订单已发货");
    ...
    private void delivery(StoreOrderSendRequest request, StoreOrder storeOrder) {
        if (StrUtil.isBlank(request.getDeliveryName())) throw new QianxuException("请输入送货人姓名");
        if (StrUtil.isBlank(request.getDeliveryTel())) throw new QianxuException("请输入送货人电话号码");
        ...
        if (!execute) throw new QianxuException("订单更新送货失败");
    ...
    private void pushMessageDeliveryOrder(StoreOrder storeOrder, User user, StoreOrderSendRequest request, List<String> productNameList) {
        ...
        if (storeOrder.getIsChannel().equals(Constants.ORDER_PAY_CHANNEL_PUBLIC) && notification.getIsWechat().equals(1)) {
            ...
            map.put("keyword2", QianxuDateUtil.dateToStr(storeOrder.getCreateTime(), Constants.DATE_FORMAT));
...
//        map.put("phone_number10", request.getDeliveryTel());
    ...
    private void virtual(StoreOrderSendRequest request, StoreOrder storeOrder) {
        ...
        if (!execute) throw new QianxuException("虚拟物品发货失败");
    }
    ...
    private Integer getCount(String dateLimit, String status) {
        ...
        if (StrUtil.isNotBlank(dateLimit)) {
            DateLimitUtilVo dateLimitUtilVo = QianxuDateUtil.getDateLimit(dateLimit);
     ...
     * @return Integer
     */
    private Integer getCount(StoreOrderTabsNumRequest request, String status) {
        ...
        return dao.getAdminOrderCount(map);
    }
