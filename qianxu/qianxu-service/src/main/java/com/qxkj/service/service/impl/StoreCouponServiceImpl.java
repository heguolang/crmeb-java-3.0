...
import com.qxkj.service.dao.StoreCouponDao;
    ...
    */
    ...
    public boolean create(StoreCouponRequest request) {
        ...
        if (storeCoupon.getIsLimited() && (storeCoupon.getTotal() == null || storeCoupon.getTotal() == 0)) {
            throw new QianxuException("请输入数量！");
        }

        if (request.getUseType() > 1 && (StrUtil.isBlank(request.getPrimaryKey()))) {
            throw new QianxuException("请选择商品/分类！");
        }

        storeCoupon.setLastTotal(storeCoupon.getTotal());
        if (!request.getIsForever()) {
            storeCoupon.setReceiveStartTime(QianxuDateUtil.nowDateTime()); //开始时间设置为当前时间
        }else{
            if (storeCoupon.getReceiveStartTime() == null || storeCoupon.getReceiveEndTime() == null) {
                throw new QianxuException("请选择领取时间范围！");
            }

            int compareDate = QianxuDateUtil.compareDate(QianxuDateUtil.dateToStr(storeCoupon.getReceiveStartTime(), Constants.DATE_FORMAT), QianxuDateUtil.dateToStr(storeCoupon.getReceiveEndTime(), Constants.DATE_FORMAT), Constants.DATE_FORMAT);
            if (compareDate > -1) {
                throw new QianxuException("请选择正确的领取时间范围！");
            }
        }
        ...
        if (!request.getIsFixedTime()) {
            if (storeCoupon.getDay() == null || storeCoupon.getDay() == 0) {
                throw new QianxuException("请输入天数！");
            }
    ...
    private void checkException(StoreCoupon storeCoupon) {
        if (storeCoupon == null || storeCoupon.getIsDel() || !storeCoupon.getStatus()) {
            throw new QianxuException("优惠券信息不存在或者已失效！");
        }

        //看是否过期
        if (!(storeCoupon.getReceiveEndTime() == null || storeCoupon.getReceiveEndTime().equals(""))) {
            //非永久可领取
            String date = QianxuDateUtil.nowDateTimeStr();
            int result = QianxuDateUtil.compareDate(date, QianxuDateUtil.dateToStr(storeCoupon.getReceiveEndTime(), Constants.DATE_FORMAT), Constants.DATE_FORMAT);
            if (result == 1) {
                //过期了
                throw new QianxuException("已超过优惠券领取最后期限！");
            }
        }
        ...
        if (storeCoupon.getIsLimited()) {
            ...
            if (storeCoupon.getLastTotal() < 1) {
                throw new QianxuException("此优惠券已经被领完了！");
            }
        }
    }
    ...
    public StoreCouponInfoResponse info(Integer id) {
        ...
        if (StrUtil.isNotBlank(storeCoupon.getPrimaryKey()) && storeCoupon.getUseType() > 1) {
            List<Integer> primaryIdList = QianxuUtil.stringToArray(storeCoupon.getPrimaryKey());
    ...
    public List<StoreCoupon> findRegisterList() {
        String dateStr = QianxuDateUtil.nowDate(Constants.DATE_FORMAT);
        ...
        List<StoreCoupon> couponList = list.stream().filter(coupon -> {
            ...
            if (ObjectUtil.isNotNull(coupon.getReceiveStartTime())) {
                //非永久可领取
                int result = QianxuDateUtil.compareDate(dateStr, QianxuDateUtil.dateToStr(coupon.getReceiveStartTime(), Constants.DATE_FORMAT), Constants.DATE_FORMAT);
                if (result == -1) {
                    // 未开始
                    return false;
                }
            ...
            // 是否有领取结束时间
            if (ObjectUtil.isNotNull(coupon.getReceiveEndTime())) {
                int compareDate = QianxuDateUtil.compareDate(dateStr, QianxuDateUtil.dateToStr(coupon.getReceiveEndTime(), Constants.DATE_FORMAT), Constants.DATE_FORMAT);
                if (compareDate > 0) {
                    return false;
                }
            }
    ...
    public List<StoreCoupon> getSendList(SearchAndPageRequest request) {
        ...
        lqw.and(o -> o.isNull(StoreCoupon::getReceiveEndTime).or().gt(StoreCoupon::getReceiveEndTime, QianxuDateUtil.nowDate(Constants.DATE_FORMAT)));
    ...
    public Boolean delete(Integer id) {
        ...
        if (ObjectUtil.isNull(coupon) || coupon.getIsDel()) {
            throw new QianxuException("优惠券不存在");
        }
     ...
     */
    @Override
    public PageInfo<StoreCouponFrontResponse> getH5List(Integer type, Integer productId, PageParamRequest pageParamRequest) {
        // 获取优惠券列表
        PageInfo<StoreCoupon> pageInfo = getListByReceive(type, productId, pageParamRequest);
        ...
        for (StoreCoupon storeCoupon : list) {
            ...
            // 更改使用时间格式，去掉时分秒
            response.setUseStartTimeStr(QianxuDateUtil.dateToStr(storeCoupon.getUseStartTime(), Constants.DATE_FORMAT_DATE));
            response.setUseEndTimeStr(QianxuDateUtil.dateToStr(storeCoupon.getUseEndTime(), Constants.DATE_FORMAT_DATE));
    ...
    public Boolean updateStatus(Integer id, Boolean status) {
        ...
        if (coupon.getStatus().equals(status)) {
            throw new QianxuException("优惠券状态无需变更");
        }
        StoreCoupon storeCoupon = new StoreCoupon();
    ...
    private void getPrimaryKeySql(LambdaQueryWrapper<StoreCoupon> lambdaQueryWrapper, String productIdStr) {
        ...
        List<Integer> categoryIdList = storeProductService.getProductAllCategoryIdByProductIds(QianxuUtil.stringToArray(productIdStr));
        lambdaQueryWrapper.and(i -> i.and(
                //通用券  商品券  品类券
                t -> t.eq(StoreCoupon::getUseType, 1)
                .or(p -> p.eq(StoreCoupon::getUseType , 2).apply(QianxuUtil.getFindInSetSql("primary_key", productIdStr)))
                .or(c -> c.eq(StoreCoupon::getUseType , 3).apply(QianxuUtil.getFindInSetSql("primary_key", (ArrayList<Integer>) categoryIdList)))
        ));
    }
}
