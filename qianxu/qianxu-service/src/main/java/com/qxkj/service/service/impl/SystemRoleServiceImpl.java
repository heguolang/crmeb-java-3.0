...
import com.qxkj.common.vo.MenuCheckTree;
import com.qxkj.common.vo.MenuCheckVo;
import com.github.pagehelper.PageHelper;
import com.qxkj.service.dao.SystemRoleDao;
import com.qxkj.service.service.CategoryService;
import com.qxkj.service.service.SystemMenuService;
    ...
    */
    ...
    public Boolean updateStatus(Integer id, Boolean status) {
        ...
        if (ObjectUtil.isNull(role)) {
            throw new QianxuException("身份不存在");
        }
    ...
    public Boolean add(SystemRoleRequest systemRoleRequest) {
        if (existName(systemRoleRequest.getRoleName(), null)) {
            throw new QianxuException("角色名称重复");
        }
    ...
    public Boolean edit(SystemRoleRequest systemRoleRequest) {
        ...
        if (ObjectUtil.isNull(role)) {
            throw new QianxuException("角色不存在");
        }
    ...
    public Boolean delete(Integer id) {
        ...
        if (ObjectUtil.isNull(systemRole)) {
            throw new QianxuException("角色已删除");
        }
        return transactionTemplate.execute(e -> {
            dao.deleteById(id);
...
//        SystemAdmin systemAdmin = systemAdminService.getInfo();
        ...
        if (null == systemAdmin || StringUtils.isBlank(systemAdmin.getRoles())){
            throw new QianxuException("没有权限访问！");
        }
        ...
        for (SystemRole systemRole : systemRoleList) {
            ...
            categoryIdList.addAll(QianxuUtil.stringToArray(systemRole.getRules()));
        }
    ...
    private List<SystemRole> getVoListInId(String roles) {
        LambdaQueryWrapper<SystemRole> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        lambdaQueryWrapper.in(SystemRole::getId, QianxuUtil.stringToArray(roles));
        return dao.selectList(lambdaQueryWrapper);
    }
