        ...
        for (int i = 0; i < size; i++) {
            ...
            try {
                ...
                if (ObjectUtil.isNull(storeOrder)) {
                    throw new QianxuException("订单不存在,orderNo = " + orderId);
                }
//                boolean result = storeOrderTaskService.refundApply(storeOrder);
        ...
        for (int i = 0; i < size; i++) {
            ...
            try {
                ...
                if (ObjectUtil.isNull(storeOrder)) {
                    ...
                    throw new QianxuException("订单不存在，orderId: " + data);
                }
        ...
        for (int i = 0; i < size; i++) {
            ...
            try {
                ...
                if (ObjectUtil.isNull(storeOrder)) {
                    ...
                    throw new QianxuException("订单不存在，orderNo: " + data);
                }
    ...
    public void autoComplete() {
        ...
        for (StoreOrder order : orderList) {
            ...
            // 判断是否到自动完成时间
            String comTime = QianxuDateUtil.addDay(orderStatus.getCreateTime(), time, Constants.DATE_FORMAT);
            int compareDate = QianxuDateUtil.compareDate(comTime, QianxuDateUtil.nowDateTime(Constants.DATE_FORMAT), Constants.DATE_FORMAT);
            if (compareDate > 0) {
                continue ;
            }
...
//                }
                ...
                reply.setCreateTime(QianxuDateUtil.nowDateTime());
                replyList.add(reply);
