...
import com.qxkj.common.request.SystemFormItemCheckRequest;
...
import com.qxkj.common.vo.SplashAdDataVo;
...
public class PageLayoutServiceImpl implements PageLayoutService {
    ...
    public Boolean save(JSONObject jsonObject) {
        ...
        if (StrUtil.isNotBlank(jsonObject.getString("indexBanner"))) {
            List<JSONObject> indexBanner = QianxuUtil.jsonArrayToJsonObjectList(jsonObject.getJSONArray("indexBanner"));
        ...
        if (StrUtil.isNotBlank(jsonObject.getString("indexMenu"))) {
            List<JSONObject> indexMenu = QianxuUtil.jsonArrayToJsonObjectList(jsonObject.getJSONArray("indexMenu"));
        ...
        if (StrUtil.isNotBlank(jsonObject.getString("indexNews"))) {
            List<JSONObject> indexNews = QianxuUtil.jsonArrayToJsonObjectList(jsonObject.getJSONArray("indexNews"));
        ...
        if (StrUtil.isNotBlank(jsonObject.getString("userMenu"))) {
            List<JSONObject> userMenu = QianxuUtil.jsonArrayToJsonObjectList(jsonObject.getJSONArray("userMenu"));
        ...
        if (StrUtil.isNotBlank(jsonObject.getString("userBanner"))) {
            List<JSONObject> userBanner = QianxuUtil.jsonArrayToJsonObjectList(jsonObject.getJSONArray("userBanner"));
    ...
    public Boolean indexTableSave(JSONObject jsonObject) {
        List<JSONObject> indexBanner = QianxuUtil.jsonArrayToJsonObjectList(jsonObject.getJSONArray("indexTable"));
    ...
    private List<HashMap<String, Object>> convertData(List<SystemGroupData> dataList) {
        return dataList.stream().map(data -> {
            ...
            List<SystemFormItemCheckRequest> systemFormItemCheckRequestList = QianxuUtil.jsonToListClass(jsonObject.getString("fields"), SystemFormItemCheckRequest.class);
    ...
    public Boolean bottomNavigationSave(JSONObject jsonObject) {
        ...
        if (StrUtil.isBlank(isCustom)) {
            throw new QianxuException("请选择是否自定义");
        }
        List<JSONObject> bottomNavigationList = QianxuUtil.jsonArrayToJsonObjectList(jsonObject.getJSONArray("bottomNavigationList"));
        if (CollUtil.isEmpty(bottomNavigationList)) {
            throw new QianxuException("请传入底部导航数据");
        }
