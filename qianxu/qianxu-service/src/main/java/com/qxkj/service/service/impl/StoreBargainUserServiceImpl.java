...
import com.qxkj.common.model.bargain.StoreBargainUserHelp;
...
import com.qxkj.service.dao.StoreBargainUserDao;
    ...
    */
    ...
    public PageInfo<StoreBargainUserResponse> getList(StoreBargainUserSearchRequest request) {
        ...
        if (StrUtil.isNotBlank(request.getDateLimit())) {
            ...
            if (StrUtil.isNotBlank(dateLimit.getStartTime())) {
                map.put("startTime",  QianxuDateUtil.dateStr2Timestamp(dateLimit.getStartTime(), Constants.DATE_TIME_TYPE_BEGIN));
                map.put("endTime", QianxuDateUtil.dateStr2Timestamp(dateLimit.getEndTime(), Constants.DATE_TIME_TYPE_END));
            }
        }
        ...
        List<StoreBargainUserResponse> list = responseList.stream().map(bargainUser -> {
            ...
            bargainUserResponse.setAddTime(QianxuDateUtil.timestamp2DateStr(Long.valueOf(bargainUser.getAddTime()), Constants.DATE_FORMAT));
            ...
            bargainUserResponse.setDataTime(QianxuDateUtil.timestamp2DateStr(storeBargain.getStopTime(), Constants.DATE_FORMAT));
    ...
    private BargainUserInfoResponse otherBargainActivity(BargainFrontRequest bargainFrontRequest) {
        ...
        if (bargainUser.getUid().equals(user.getUid())) {// 自己的砍价活动
            ...
            percent =  QianxuUtil.percentInstanceIntVal(alreadyPrice, alreadyPrice.add(surplusPrice));
            infoResponse.setBargainStatus(bargainStatus);
            infoResponse.setAlreadyPrice(alreadyPrice);
            infoResponse.setSurplusPrice(surplusPrice);
            infoResponse.setBargainPercent(percent);
    ...
    private BargainUserInfoResponse oneselfBargainActivity(BargainFrontRequest bargainFrontRequest) {
        ...
        if (storeBargainUser.getStatus().equals(2)) {// 活动结束，砍价未完成
            throw new QianxuException("未在活动期内完成砍价");
        }
        // 参与过该商品的砍价
