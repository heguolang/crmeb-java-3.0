...
import com.qxkj.common.model.product.StoreProductGroup;
import com.qxkj.common.model.product.StoreProductGroupRel;
...
import com.qxkj.common.request.StoreProductGroupRequest;
import com.qxkj.common.request.StoreProductGroupSearchRequest;
import com.qxkj.common.utils.QianxuDateUtil;
import com.qxkj.common.utils.QianxuUtil;
import com.qxkj.service.dao.AgentDao;
import com.qxkj.service.dao.StoreProductGroupDao;
import com.qxkj.service.dao.StoreProductGroupRelDao;
...
public class StoreProductGroupServiceImpl extends ServiceImpl<StoreProductGroupDao, StoreProductGroup>
    ...
    public Boolean create(StoreProductGroupRequest request) {
        ...
        if (!save(group)) {
            throw new QianxuException("创建商品分组失败");
        }
    ...
    public Boolean edit(Integer id, StoreProductGroupRequest request) {
        ...
        group.setUpdateTime(new Date());
        if (!updateById(group)) {
            throw new QianxuException("更新商品分组失败");
        }
    ...
    public void assertPurchaseAllowed(User user, Integer productId, Integer buyNum) {
        if (!canBuyProduct(user, productId)) {
            throw new QianxuException(getDenyTip());
        }
        ...
        if (num < minBuy) {
            throw new QianxuException("该商品起卖数为" + minBuy);
        }
        if (limitOne && num > 1) {
            throw new QianxuException("该商品限购一件");
        }
    }
    ...
    public Boolean batchBindProducts(List<Integer> productIds, List<Integer> groupIds) {
        if (CollUtil.isEmpty(productIds) || CollUtil.isEmpty(groupIds)) {
            throw new QianxuException("请选择商品和分组");
        }
    ...
    private boolean matchUserGroup(User user, String userGroupIds) {
        ...
        List<Integer> required = QianxuUtil.stringToArray(userGroupIds);
        List<Integer> owned = QianxuUtil.stringToArray(user.getGroupId());
    ...
    private boolean containsLevel(String ids, Integer levelId) {
        ...
        return QianxuUtil.stringToArray(ids).contains(levelId);
    }
    ...
    private StoreProductGroup getValidGroup(Integer id) {
        ...
        if (group == null || Boolean.TRUE.equals(group.getIsDel())) {
            throw new QianxuException("商品分组不存在");
        }
        return group;
    }
    ...
    public Integer ensureTheme(Integer groupId) {
        ...
        int now = QianxuDateUtil.getNowTime();
        ...
        if (themeDao.insert(theme) <= 0 || ObjectUtil.isNull(theme.getId())) {
            throw new QianxuException("创建分组装修页失败");
        }
    ...
    private void validatePermissionType(String type) {
        if (!StoreProductGroup.PERM_ALL.equals(type)
            ...
            throw new QianxuException("权限类型不正确");
        }
    }
