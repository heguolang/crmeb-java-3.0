...
import com.qxkj.service.dao.StoreSeckillDao;
...
//        String currentSkillTime = null;
        ...
        for (StoreSeckill product : storeProducts) {
            ...
            storeProductResponse.setImages(QianxuUtil.stringToArrayStr(product.getImages()));
    ...
    public Boolean saveSeckill(StoreSeckillAddRequest request) {
        request.getAttrValue().forEach(e -> {
            if (e.getStock() <= 0) {
                throw new QianxuException("库存不足的规格不能参与秒杀活动");
            }
        ...
        if (isExistTile(request.getTitle())) {
            throw new QianxuException("活动标题已经存在");
        }
        ...
        // 设置秒杀开始时间和结束时间
        storeSeckill.setStartTime(QianxuDateUtil.strToDate(request.getStartTime(), Constants.DATE_FORMAT_DATE));
        storeSeckill.setStopTime(QianxuDateUtil.strToDate(request.getStopTime(), Constants.DATE_FORMAT_DATE));
    ...
    public Boolean updateSeckill(StoreSeckillAddRequest request) {
        ...
        if (ObjectUtil.isNull(storeSeckill) || storeSeckill.getIsDel()) {
            throw new QianxuException("秒杀商品不存在");
        }

        if (storeSeckill.getStatus().equals(1)) {
            throw new QianxuException("请先关闭秒杀商品，再修改商品信息");
        }


        StoreSeckill seckill = new StoreSeckill();
        BeanUtils.copyProperties(request, seckill);
        seckill.setStartTime(QianxuDateUtil.strToDate(request.getStartTime(),Constants.DATE_FORMAT_DATE));
        seckill.setStopTime(QianxuDateUtil.strToDate(request.getStopTime(),Constants.DATE_FORMAT_DATE));
    ...
    public StoreSeckillDetailResponse getDetailH5(Integer skillId) {
        ...
        if (ObjectUtil.isNull(storeSeckill) || storeSeckill.getIsDel()) {
            throw new QianxuException("未找到对应秒杀商品信息");
        }
        if (storeSeckill.getStatus().equals(0)) {
            throw new QianxuException("秒杀商品已下架");
        }
...
//        detailH5Response.setStoreInfo(storeSeckill.getInfo());
        ...
        if (StrUtil.isNotBlank(storeProduct.getGuaranteeIds())) {
            productDetailResponse.setGuaranteeList(guaranteeService.findByIdList(QianxuUtil.stringToArray(storeProduct.getGuaranteeIds())));
        }
        // 主商品状态
        ...
        if (ObjectUtil.isNotNull(seckillManger)) {
            int secKillEndSecondTimestamp = QianxuDateUtil.getSecondTimestamp(QianxuDateUtil.nowDateTime("yyyy-MM-dd " + seckillManger.getEndTime() + ":00:00"));
...
//        infoResponse.setStoreInfo(storeSeckill.getInfo());
    ...
    public List<SecKillResponse> getForH5Index() {
        ...
        skillManagerList.forEach(e->{
            ...
            if (proNum > 0) {
                int secKillEndSecondTimestamp = QianxuDateUtil.getSecondTimestamp(QianxuDateUtil.nowDateTime("yyyy-MM-dd " + e.getEndTime() + ":00:00"));
    ...
    public List<StoreSecKillH5Response> getKillListByTimeId(String timeId, PageParamRequest pageParamRequest) {
        ...
        storeSeckills.forEach(e->{
            ...
            response.setPercent(QianxuUtil.percentInstanceIntVal(e.getQuotaShow() - e.getQuota(), e.getQuotaShow()));
    ...
    public StoreSeckill getByIdException(Integer id) {
        ...
        lqw.eq(StoreSeckill::getIsShow, true);
        StoreSeckill storeSeckill = dao.selectOne(lqw);
        if (ObjectUtil.isNull(storeSeckill) || storeSeckill.getIsDel()) throw new QianxuException("秒杀商品不存在或以删除");
        return storeSeckill;
    }
    ...
    public Boolean operationStock(Integer id, Integer num, String type) {
        ...
        if (!update) {
            throw new QianxuException("更新商品库存失败！商品id = " + id);
        }
        return update;
    }
