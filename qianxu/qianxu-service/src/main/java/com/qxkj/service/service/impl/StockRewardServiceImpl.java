...
import com.qxkj.common.model.stock.StockReward;
import com.qxkj.common.model.stock.StockWithdraw;
...
import com.qxkj.service.dao.StockLadderDao;
import com.qxkj.service.dao.StockNoticeDao;
import com.qxkj.service.dao.StockOrderDao;
import com.qxkj.service.dao.StockOrderProductDao;
import com.qxkj.service.dao.StockRewardDao;
import com.qxkj.service.dao.StockWithdrawDao;
...
public class StockRewardServiceImpl implements StockRewardService {
    ...
    @Resource
    private com.qxkj.service.service.UserBrokerageRecordService userBrokerageRecordService;

    @Resource
    private com.qxkj.service.dao.StockLevelDao stockLevelDao;
     ...
     */
    @Override
    public void settleExchangeDiffReward(com.qxkj.common.model.stock.StockExchange exchange) {
        if (exchange == null || exchange.getDiffPrice() == null || exchange.getDiffPrice().signum() <= 0) {
            return;
        }
        // 必须换货单已完成且差价已支付
        if (!com.qxkj.common.model.stock.StockExchange.STATUS_COMPLETE.equals(exchange.getStatus())) {
            return;
        }
    ...
    private void calcPeerReward(StockOrder order) {
        ...
        com.qxkj.common.model.stock.StockLevel level = stockLevelDao.selectById(up.getLevelId());
    ...
    public Boolean settleLadderReward(Integer type, String month) {
        // 团队奖开关：关闭时阶梯奖同样停发
        if (!"1".equals(systemConfigService.getValueByKey("sys_switch_team_reward"))) {
            throw new QianxuException("团队奖未开启");
        }
        if (!"1".equals(systemConfigService.getValueByKey("stock_ladder_status"))) {
            throw new QianxuException("阶梯业绩奖励未开启");
        }
    ...
    private java.util.Date parseMonth(String month) {
        try {
            ...
            throw new QianxuException("月份格式错误：" + month);
        }
    }
    ...
    public Boolean applyWithdraw(Integer uid, BigDecimal price, String mark) {
        if (price == null || price.signum() <= 0) {
            throw new QianxuException("提现金额必须大于0");
        }
        StockAgent agent = stockService.getAgentByUid(uid);
        ...
        if (price.compareTo(balance) > 0) {
            throw new QianxuException("超出可提现余额，当前可提现 ¥" + balance);
        }
    ...
    public Boolean readNotice(Integer uid, Integer noticeId) {
        ...
        if (notice == null || !notice.getUid().equals(uid)) {
            throw new QianxuException("消息不存在");
        }
        notice.setIsRead(1);
    ...
    public Boolean auditWithdraw(Integer withdrawId, StockRequests.StockWithdrawAuditRequest request) {
        ...
        if (withdraw == null) {
            throw new QianxuException("提现单不存在");
        }
        if (!withdraw.getStatus().equals(StockWithdraw.STATUS_WAIT_AUDIT)) {
            throw new QianxuException("该提现单已审核");
        }
    ...
    public HashMap<String, Object> getReport(Integer uid, String dateLimit, PageParamRequest page) {
        ...
        com.qxkj.common.page.CommonPage<StockAgent> agentPage =
    ...
    private void creditBrokerage(Integer uid, BigDecimal price, String linkId, String title, String mark) {
        ...
        record.setCreateTime(com.qxkj.common.utils.QianxuDateUtil.nowDateTime());
