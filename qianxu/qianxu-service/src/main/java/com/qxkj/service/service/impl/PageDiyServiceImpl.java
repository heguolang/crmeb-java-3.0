...
import com.qxkj.common.model.page.PageDiy;
import com.qxkj.common.request.PageParamRequest;
import com.qxkj.common.request.page.PageDiyEditNameRequest;
import com.qxkj.common.response.page.PageDiyResponse;
import com.qxkj.common.result.CommonResultCode;
import com.qxkj.common.result.SystemConfigResultCode;
import com.qxkj.service.dao.page.PageDiyDao;
import com.qxkj.service.service.PageDiyService;
...
public class PageDiyServiceImpl extends ServiceImpl<PageDiyDao, PageDiy> implements PageDiyService {
    ...
    public PageDiy savePageDiy(PageDiy pageDiy) {
        ...
        if (StrUtil.isBlank(adminApiPath)) {
            throw new QianxuException(CommonResultCode.VALIDATE_FAILED, "应用设置中 微信小程序数据配置 或者 支付回调地址以及网站地址 配置不全");
        }
        // 检查diy模版名称唯一
...
//        pageDiy.setValue(jsonContext.jsonString());
    ...
    public Boolean setDiyPageHome(Integer diyId) {
        ...
        if(ObjectUtil.isNull(pageDiy)) throw new QianxuException(CommonResultCode.VALIDATE_FAILED, "当前DIY模版不存在");
    ...
    public PageDiy getDiyPageHome(Boolean isLoadValue) {
        ...
        if(ObjectUtil.isNull(currentWaitResetPageHome) || currentWaitResetPageHome.size() != 1){
            throw new QianxuException("首页模版设置不正确！");
        }
...
//            pageDiy.setValue(modifiedJsonString);
    ...
    public PageDiy getDiyPageByPageIdForAdmin(Integer id) {
        ...
        if(ObjectUtil.isNull(pageDiy)) throw new QianxuException(SystemConfigResultCode.PAGE_DIY_NOT_EXIST);
...
//        response.setValue(JSON.parseObject(modifiedJsonString));
    ...
    private void checkPageDiyNameUnique(String pageDiyName, Integer id) {
        ...
        if(ObjectUtil.isNotNull(pageDiyNameExist) && !pageDiyNameExist.isEmpty()){
            throw new QianxuException(CommonResultCode.VALIDATE_FAILED, "当前模版名称已经存在，请修改后再保存！");
        }
    }
