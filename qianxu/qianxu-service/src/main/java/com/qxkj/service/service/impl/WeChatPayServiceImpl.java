...
public class WeChatPayServiceImpl implements WeChatPayService {
    ...
    public Boolean queryPayResult(String orderNo) {
        if (StrUtil.isBlank(orderNo)) {
            throw new QianxuException("订单编号不能为空");
        }
        // 切割字符串，判断是支付订单还是充值订单
        ...
        if (pre.equals("order")) {// 支付订单
            ...
            if (!storeOrder.getPayType().equals(PayConstants.PAY_TYPE_WE_CHAT)) {
                throw new QianxuException("不是微信支付类型订单，请重新选择支付方式");
            }
            ...
            if (ObjectUtil.isNull(wechatPayInfo)) {
                throw new QianxuException("未找到对应微信订单");
            }
            ...
            if (!updatePaid) {
                throw new QianxuException("支付成功更新订单失败");
            }
            // 添加支付成功task
        ...
        if (!rechargePayAfter) {
            throw new QianxuException("wechat pay error : 数据保存失败==》" + orderNo);
        }
