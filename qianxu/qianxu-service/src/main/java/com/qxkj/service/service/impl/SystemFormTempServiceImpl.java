...
import com.qxkj.common.vo.SystemConfigFormItemConfigRegListVo;
import com.qxkj.common.vo.SystemConfigFormItemVo;
import com.qxkj.common.vo.SystemConfigFormVo;
import com.github.pagehelper.PageHelper;
import com.qxkj.common.utils.ValidateFormUtil;
import com.qxkj.common.model.system.SystemFormTemp;
import com.qxkj.service.dao.SystemFormTempDao;
    ...
    */
    ...
    public void checkForm(SystemFormCheckRequest systemFormCheckRequest) {
        ...
        try {
            ...
            throw new QianxuException("模板表单 【" + formTemp.getName() + "】 的内容不是正确的JSON格式！");
        }
        ...
        for (String item : systemConfigFormVo.getFields()) {
            ...
            if(systemConfigFormItemVo.get__config__().getRequired() && StringUtils.isEmpty(map.get(model))) {
                throw new QianxuException(systemConfigFormItemVo.get__config__().getLabel() + "不能为空！");
            }
            //正则验证
    ...
    public Boolean add(SystemFormTempRequest systemFormTempRequest) {
        try {
            ...
            throw new QianxuException("模板表单 【" + systemFormTempRequest.getName() + "】 的内容不是正确的JSON格式！");
        }
    ...
    private void checkRule(List<SystemConfigFormItemConfigRegListVo> regList, String value, String name) {
        ...
        for (SystemConfigFormItemConfigRegListVo systemConfigFormItemConfigRegListVo : regList) {
            if(!ValidateFormUtil.regular(value, name, systemConfigFormItemConfigRegListVo.getPattern())) {
                throw new QianxuException(systemConfigFormItemConfigRegListVo.getMessage());
            }
        }
    }
}
