...
import com.qxkj.common.model.system.SystemUserLevelBrokerage;
...
import com.qxkj.common.utils.WxPayUtil;
...
public class OrderPayServiceImpl implements OrderPayService {
    ...
    public Boolean paySuccess(StoreOrder storeOrder) {
        ...
        int integralBalanceCursor = integralBeforeCredit;
        int totalIntegralAddTemp = 0;
        if (CollUtil.isNotEmpty(integralList)) {
            for (UserIntegralRecord integralRecord : integralList) {
                ...
                int addAmount = ObjectUtil.defaultIfNull(integralRecord.getIntegral(), 0);
                totalIntegralAddTemp += addAmount;
                integralBalanceCursor += addAmount;
                integralRecord.setBalance(integralBalanceCursor);
                integralRecord.setStatus(IntegralRecordConstants.INTEGRAL_RECORD_STATUS_COMPLETE);
                integralRecord.setFrozenTime(0);
            }
        }
        final int totalIntegralAdd = totalIntegralAddTemp;
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            if (totalIntegralAdd > 0) {
                ...
                if (!Boolean.TRUE.equals(integralOk)) {
                    throw new QianxuException("积分到账失败");
                }
            }
            ...
            if (CollUtil.isNotEmpty(creditNowBrokerageList)) {
                ...
                for (UserBrokerageRecord br : creditNowBrokerageList) {
                    ...
                    if (!Boolean.TRUE.equals(brokerageOk)) {
                        throw new QianxuException("佣金到账失败");
                    }
        ...
        if (execute) {
            if (storeOrder.getType().equals(1)) {// 视频订单
                ...
                if (ObjectUtil.isNull(componentOrder)) {
                    throw new QianxuException("组件订单未找到,订单号 = " + storeOrder.getOrderId());
                }
            ...
            try {
                ...
                if (StrUtil.isNotBlank(user.getPhone()) && payNotification.getIsSms().equals(1)) {
                    ...
                    smsService.sendPaySuccess(user.getPhone(), storeOrder.getOrderId(), storeOrder.getPayPrice(), smsTemplate.getTempId());
                }
                ...
                if (payAdminNotification.getIsSms().equals(1)) {
                    ...
                    if (CollUtil.isNotEmpty(systemAdminList)) {
                        ...
                        systemAdminList.forEach(admin -> {
                            smsService.sendOrderPaySuccessNotice(admin.getPhone(), storeOrder.getOrderId(), admin.getRealName(), smsTemplate.getTempId());
                        });
                    }
                }
                ...
                // 到账方式为「完成到账」时保留 CREATE 记录，支付节点不冻结；支付到账记录已为 COMPLETE，冻结无影响
...
//        temMap.put("thing5", "恭喜您拼团成功！我们将尽快为您发货。");
    ...
    private List<UserBrokerageRecord> assignCommission(StoreOrder storeOrder) {
        ...
        List<UserBrokerageRecord> brokerageRecordList = spreadRecordList.stream().map(record -> {
            ...
            brokerageRecord.setCreateTime(QianxuDateUtil.nowDateTime());
    ...
    private Boolean yuePay(StoreOrder storeOrder) {
        ...
        // 用户余额扣除
        User user = userService.getById(storeOrder.getUid());
        if (ObjectUtil.isNull(user)) throw new QianxuException("用户不存在");
        if (user.getNowMoney().compareTo(storeOrder.getPayPrice()) < 0) {
            throw new QianxuException("用户余额不足");
        }
        if (user.getIntegral() < storeOrder.getUseIntegral()) {
            throw new QianxuException("用户积分不足");
        }
        storeOrder.setPaid(true);
        storeOrder.setPayTime(QianxuDateUtil.nowDateTime());
        ...
        if (!execute) throw new QianxuException("余额支付订单失败");
        return execute;
    }
    ...
    public OrderPayResultResponse payment(OrderPayRequest orderPayRequest, String ip) {
        ...
        if (storeOrder.getIsDel()) {
            throw new QianxuException("订单已被删除");
        }
        if (storeOrder.getPaid()) {
            throw new QianxuException("订单已支付");
        }
        User user = userService.getById(storeOrder.getUid());
        if (ObjectUtil.isNull(user)) throw new QianxuException("用户不存在");
...
//        }
        ...
        if (!changePayType) {
            throw new QianxuException("变更订单支付类型失败!");
        }
...
//                String encode;
                ...
                try {
                    ...
                    e.printStackTrace();
                    throw new QianxuException("支付宝参数UrlEncode异常");
                }
                ...
                try {
                    ...
                    throw new QianxuException(e.getErrMsg());
                }
...
//            model.setBody(body);
        ...
        if (storeOrder.getPayType().equals(PayConstants.PAY_TYPE_OFFLINE)) {
            throw new QianxuException("暂时不支持线下支付");
        }
    ...
    private Map<String, String> unifiedorder(StoreOrder storeOrder, String ip) {
        ...
        if (ObjectUtil.isNull(userToken)) {
            throw new QianxuException("该用户没有openId");
        }
        ...
        map.put("prepayTime", QianxuDateUtil.nowDateTimeStr());
    ...
    private CreateOrderRequestVo getUnifiedorderVo(StoreOrder storeOrder, String openid, String ip, String appId, String mchId, String signKey) {
        ...
        vo.setOut_trade_no(QianxuUtil.getOrderNo("wxNo"));
    ...
    private String componentCreateOrder(StoreOrder storeOrder, User user, Map<String, String> unifiedorder) {
        ...
        orderInfoList.forEach(orderInfo -> {
            ...
            if (ObjectUtil.isNull(product)) {
                throw new QianxuException("订单商品未找到");
            }
            ...
            if (ObjectUtil.isNull(productSku)) {
                throw new QianxuException("订单商品sku未找到");
            }
    ...
    private UserIntegralRecord integralRecordInit(StoreOrder storeOrder, Integer balance, Integer integral, String type) {
        ...
        integralRecord.setCreateTime(QianxuDateUtil.nowDateTime());
        return integralRecord;
    }
