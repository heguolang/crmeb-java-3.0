...
import com.qxkj.common.vo.UploadCommonVo;
...
import com.qxkj.common.utils.UploadUtil;
...
public class UploadServiceImpl implements UploadService {
    ...
    public FileResultVo imageUpload(MultipartFile multipartFile, String model, Integer pid) {
        ...
        try {
            ...
            throw new QianxuException("图片上传 IO异常");
        }
        return fileResultVo;
    }
    ...
    public FileResultVo fileUpload(MultipartFile multipartFile, String model, Integer pid) throws IOException {
        ...
        try {
            ...
            throw new QianxuException("文件上传 IO异常");
        }
        return fileResultVo;
    }
    ...
    private String uploadValidate(String fileName, float fileSize, String fileType, String contentType, boolean isFile) {
        ...
        if (StrUtil.isEmpty(extName)) {
            if (StrUtil.isNotBlank(contentType)) {
                ...
                throw new QianxuException(CommonResultCode.VALIDATE_FAILED, "文件类型未定义，无法上传...");
            }
        }
        ...
        if (StrUtil.isNotBlank(extStr)) {
            // 切割文件扩展名
            List<String> extensionList = QianxuUtil.stringToArrayStr(extStr);
            if (CollUtil.isNotEmpty(extensionList)) {
                //判断
                if (!extensionList.contains(extName)) {
                    throw new QianxuException(CommonResultCode.VALIDATE_FAILED, "上载文件类型只能为：" + extStr);
                }
            } else {
                throw new QianxuException(CommonResultCode.VALIDATE_FAILED, "上载文件类型只能为：" + extStr);
            }
        ...
        if (fileSize > size) {
            throw new QianxuException(CommonResultCode.VALIDATE_FAILED, StrUtil.format("最大允许上传 {} MB文件，当前文件大小为 {} MB", size, fs));
        }
        return extName;
    }
    ...
    private FileResultVo commonUpload(MultipartFile multipartFile, String model, Integer pid, String fileType, boolean isFile) throws IOException {
        if (ObjectUtil.isNull(multipartFile) || multipartFile.isEmpty()) {
            throw new QianxuException(CommonResultCode.VALIDATE_FAILED, "上载的文件对象不存在...");
        }
        // 校验
        ...
        // 服务器存储地址
        String rootPath = qianxuConfig.getImagePath().trim();
        // 模块
        ...
        String webPath = type + modelPath + QianxuDateUtil.nowDate("yyyy/MM/dd") + "/";
        // 文件分隔符转化为当前系统的格式
...
//                    }
                    ...
                    String webPathQn = qianxuConfig.getImagePath();
            ...
            case 3:
                ...
                try {
                    String webPathAl = qianxuConfig.getImagePath();
            ...
            case 4:
                ...
                try {
                    String webPathTx = qianxuConfig.getImagePath();
