...
import com.qxkj.common.vo.OrderInfoVo;
import com.qxkj.service.dao.StoreCouponUserDao;
...
public class StoreCouponUserServiceImpl extends ServiceImpl<StoreCouponUserDao, StoreCouponUser> implements StoreCouponUserService {
    ...
    public Boolean receive(StoreCouponUserRequest request) {
        ...
        List<Integer> uidList = QianxuUtil.stringToArray(request.getUid());
        if (uidList.size() < 1) {
            throw new QianxuException("请选择用户！");
        }
        ...
        if (storeCoupon.getIsLimited()) {
            ...
            if (storeCoupon.getLastTotal() < uidList.size()) {
                throw new QianxuException("当前剩余数量不够领取！");
            }
        }
        ...
        if (uidList.size() < 1) {
            //都已经领取过了
            throw new QianxuException("当前用户已经领取过此优惠券了！");
        }
    ...
    public void overdueTask() {
        ...
        String nowDateStr = QianxuDateUtil.nowDate(Constants.DATE_FORMAT);
        couponList.forEach(coupon -> {
            if (ObjectUtil.isNotNull(coupon.getEndTime())) {
                String endDateStr = QianxuDateUtil.dateToStr(coupon.getEndTime(), Constants.DATE_FORMAT);
                if (QianxuDateUtil.compareDate(nowDateStr, endDateStr, Constants.DATE_FORMAT) >= 0) {
                    coupon.setStatus(2);
        ...
        if (!update) throw new QianxuException("批量更新优惠券过期动作失败");
    }
    ...
    public MyRecord paySuccessGiveAway(Integer couponId, Integer uid) {
        ...
        if (ObjectUtil.isNotNull(storeCoupon.getReceiveStartTime())) {
            ...
            int result = QianxuDateUtil.compareDate(date, QianxuDateUtil.dateToStr(storeCoupon.getReceiveStartTime(), Constants.DATE_FORMAT), Constants.DATE_FORMAT);
            if (result == -1) {
                // 未开始
    ...
    public CommonPage<StoreCouponUserResponse> getMyCouponList(String type, PageParamRequest pageParamRequest) {
        ...
        for (StoreCouponUser storeCouponUser : couponUserList) {
            ...
            // 更改使用时间格式，去掉时分秒
            storeCouponUserResponse.setUseStartTimeStr(QianxuDateUtil.dateToStr(storeCouponUserResponse.getStartTime(), Constants.DATE_FORMAT_DATE));
            storeCouponUserResponse.setUseEndTimeStr(QianxuDateUtil.dateToStr(storeCouponUserResponse.getEndTime(), Constants.DATE_FORMAT_DATE));
