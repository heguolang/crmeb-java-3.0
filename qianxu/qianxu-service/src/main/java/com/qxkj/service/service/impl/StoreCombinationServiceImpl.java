...
import com.qxkj.common.constants.ProductConstants;
...
import com.qxkj.service.dao.StoreCombinationDao;
...
public class StoreCombinationServiceImpl extends ServiceImpl<StoreCombinationDao, StoreCombination> implements StoreCombinationService {
    ...
    public PageInfo<StoreCombinationResponse> getList(StoreCombinationSearchRequest request, PageParamRequest pageParamRequest) {
        ...
        List<StoreCombinationResponse> responseList = storeCombinationList.stream().map(combination -> {
            ...
            combinationResponse.setStopTimeStr(QianxuDateUtil.timestamp2DateStr(combination.getStopTime(), Constants.DATE_FORMAT_DATE));
    ...
    public Boolean saveCombination(StoreCombinationRequest request) {
        ...
        request.getAttrValue().forEach(e -> {
            if (e.getStock() <= 0) {
                throw new QianxuException("库存不足的规格不能参与拼团活动");
            }
            if ((ObjectUtil.isNull(e.getQuota()) || e.getQuota() > e.getStock())) {
                throw new QianxuException("请规范活动限购数量：必须小于等于库存数量");
            }
        });
        // 过滤掉checked=false的数据
//        clearNotCheckedAndValidationPrice(request);
        ...
        // 校验结束时间
        Long stopTime = QianxuDateUtil.dateStr2Timestamp(request.getStopTime(), Constants.DATE_TIME_TYPE_END);
        if (stopTime <= System.currentTimeMillis()) {
            throw new QianxuException("活动结束时间不能小于当前时间");
        }
        ...
        // 活动开始结束时间
        storeCombination.setStartTime(QianxuDateUtil.dateStr2Timestamp(request.getStartTime(), Constants.DATE_TIME_TYPE_BEGIN));
    ...
    public Boolean deleteById(Integer id) {
        ...
        if (combination.getIsShow().equals(true) && combination.getStartTime() <= timeMillis && timeMillis <= combination.getStopTime()) {
            throw new QianxuException("活动开启中，商品不支持删除");
        }
    ...
    public Boolean updateCombination(StoreCombinationRequest request) {
        ...
        if (existCombination.getIsShow().equals(true) && existCombination.getStartTime() <= timeMillis && timeMillis <= existCombination.getStopTime()) {
            throw new QianxuException("活动开启中，商品不支持修改");
        }
        ...
        // 活动开始结束时间
        storeCombination.setStartTime(QianxuDateUtil.dateStr2Timestamp(request.getStartTime(), Constants.DATE_TIME_TYPE_BEGIN));
        storeCombination.setStopTime(QianxuDateUtil.dateStr2Timestamp(request.getStopTime(), Constants.DATE_TIME_TYPE_END));
...
//        storeProductResponse.setStopTimeStr(DateUtil.timestamp2DateStr(storeCombination.getStopTime(), Constants.DATE_FORMAT_DATE));
    ...
    public Boolean updateCombinationShow(Integer id, Boolean isShow) {
        ...
        if (isShow) {
            ...
            if (ObjectUtil.isNull(product)) {
                throw new QianxuException("关联的商品已删除，无法开启活动");
            }
        }
    ...
    public CombinationDetailResponse getH5Detail(Integer comId) {
        ...
        if (ObjectUtil.isNull(storeCombination) || storeCombination.getIsDel()) {
            throw new QianxuException("对应拼团商品不存在");
        }
        if (!storeCombination.getIsShow()) {
            throw new QianxuException("拼团商品已下架");
        }
...
//        infoResponse.setStoreInfo(storeCombination.getInfo());
        ...
        if (StrUtil.isNotBlank(storeProduct.getGuaranteeIds())) {
            detailResponse.setGuaranteeList(guaranteeService.findByIdList(QianxuUtil.stringToArray(storeProduct.getGuaranteeIds())));
        }
        // 主商品状态
    ...
    public GoPinkResponse goPink(Integer pinkId) {
        ...
        if (ObjectUtil.isNull(teamPink) || teamPink.getIsRefund()) throw new QianxuException("对应的拼团不存在");
        StoreCombination storeCombination = getById(teamPink.getCid());
        if (ObjectUtil.isNull(storeCombination) || storeCombination.getIsDel()) throw new QianxuException("对应拼团商品不存在");

        // 判断拼团活动时效
        if (!storeCombination.getIsShow()) {
            throw new QianxuException("拼团活动已结束");
        }
    ...
    public Boolean removePink(StorePinkRequest storePinkRequest) {
        ...
        if (ObjectUtil.isNull(userPink)) throw new QianxuException("未查到拼团信息，无法取消");
        if (userPink.getIsRefund()) throw new QianxuException("拼团订单已退款");
        // 获取是否拼团成功
        if (userPink.getStatus() == 2) {
            throw new QianxuException("拼团已完成，无法取消");
        }
        ...
        if (userPink.getStatus() == 3) {
            throw new QianxuException("拼团已申请取消");
        }

        StoreOrder order = storeOrderService.getByOderId(userPink.getOrderId());
        if (ObjectUtil.isNull(order) || order.getIsDel()) throw new QianxuException("拼团订单不存在");
        if (order.getStatus() == -1 && order.getRefundStatus() != 0) {
            throw new QianxuException("拼团订单已进入退款流程");
        }
        ...
        if (!apply) throw new QianxuException("订单申请退款失败");
    ...
    public StoreCombination getByIdException(Integer combinationId) {
        ...
        if (ObjectUtil.isNull(storeCombination)) throw new QianxuException("拼团商品不存在或未开启");
        return storeCombination;
    }
    ...
    public Boolean operationStock(Integer id, Integer num, String type) {
        ...
        if (!update) {
            throw new QianxuException("更新拼团商品库存失败,商品id = " + id);
        }
        return update;
    }
    ...
    public CombinationIndexResponse getIndexInfo() {
        ...
        combinationList.forEach(e -> {
            int percentIntVal = QianxuUtil.percentInstanceIntVal(e.getQuota(), e.getQuotaShow());
