...
import com.qxkj.common.model.user.*;
...
import com.qxkj.common.result.LoginResultCode;
import com.qxkj.common.result.UserResultCode;
import com.qxkj.common.token.FrontTokenComponent;
import com.qxkj.common.utils.*;
import com.qxkj.common.vo.DateLimitUtilVo;
import com.qxkj.service.dao.UserDao;
...
public class UserServiceImpl extends ServiceImpl<UserDao, User> implements UserService {
    ...
    public PageInfo<UserResponse> getList(UserSearchRequest request) {
        ...
        if (StrUtil.isNotBlank(request.getGroupId())) {
            List<Integer> groupIdList = QianxuUtil.stringToArray(request.getGroupId());
            map.put("groupIdList", groupIdList);
        }

        if (!StringUtils.isBlank(request.getLabelId())) {
            String tagIdSql = QianxuUtil.getFindInSetSql("u.tag_id", request.getLabelId());
            map.put("tagIdSql", tagIdSql);
        }

        if (StrUtil.isNotBlank(request.getLevel())) {
            List<Integer> levelList = QianxuUtil.stringToArray(request.getLevel());
        ...
        for (User user : userList) {
            ...
            // 手机号脱敏处理
            if (qianxuConfig.getDemoSite()) {
                userResponse.setPhone(QianxuUtil.maskMobile(userResponse.getPhone()));
            }
    ...
    public Boolean updateIntegralMoney(UserOperateIntegralMoneyRequest request) {
        ...
        if (request.getMoneyType().equals(2) && request.getMoneyValue().compareTo(BigDecimal.ZERO) != 0) {
            if (user.getNowMoney().subtract(request.getMoneyValue()).compareTo(BigDecimal.ZERO) < 0) {
                throw new QianxuException("余额扣减后不能小于0");
            }
        }
        ...
        if (!execute) {
            throw new QianxuException("修改积分/余额失败");
        }
        return execute;
    }
    ...
    public Boolean updateBrokerage(UserOperateBrokerageRequest request) {
        if (ObjectUtil.isNull(request.getBrokerageValue()) || request.getBrokerageValue().compareTo(BigDecimal.ZERO) <= 0) {
            throw new QianxuException("修改佣金金额必须大于0");
        }
        User user = getById(request.getUid());
        if (ObjectUtil.isNull(user)) {
            throw new QianxuException("用户不存在");
        ...
        if (!execute) {
            throw new QianxuException("修改佣金失败");
        }
        return execute;
    }
    ...
    public Boolean group(String id, String groupIdValue) {
        if (StrUtil.isBlank(id)) throw new QianxuException("会员编号不能为空");
        if (StrUtil.isBlank(groupIdValue)) throw new QianxuException("分组id不能为空");

        //循环id处理
        List<Integer> idList = QianxuUtil.stringToArray(id);
        idList = idList.stream().distinct().collect(Collectors.toList());
        List<User> list = getListInUid(idList);
        if (CollUtil.isEmpty(list)) throw new QianxuException("没有找到用户信息");
        if (list.size() < idList.size()) {
            throw new QianxuException("没有找到用户信息");
        }
        for (User user : list) {
    ...
    public Boolean password(PasswordRequest request) {
        ...
        //密码
        user.setPwd(QianxuUtil.encryptPassword(request.getPassword(), user.getAccount()));
    ...
    public User getInfoException() {
        User user = getInfo();
        if (user == null) {
            throw new QianxuException("用户信息不存在！");
        }

        if (!user.getStatus()) {
            throw new QianxuException("用户已经被禁用！");
        }
        return user;
    }
    ...
    public Map<Object, Object> getAddUserCountGroupDate(String date) {
        ...
        for (User user : list) {
            map.put(QianxuDateUtil.dateToStr(user.getCreateTime(), Constants.DATE_FORMAT_DATE), user.getUid());
        }
        return map;
    }
    ...
    public Boolean updatePhoneVerify(UserBindingPhoneUpdateRequest request) {
        ...
        if (!user.getPhone().equals(request.getPhone())) {
            throw new QianxuException("手机号不是当前用户手机号");
        }
    ...
    public Boolean updatePhone(UserBindingPhoneUpdateRequest request) {
        ...
        if (null != user) {
            throw new QianxuException("此手机号码已被注册");
        }
    ...
    public UserCenterResponse getUserCenter() {
        ...
        if (ObjectUtil.isNull(currentUser)) {
            throw new QianxuException("您的登录已过期，请先登录");
        }
    ...
    public Boolean tag(String id, String tagIdValue) {
        if (StrUtil.isBlank(id)) throw new QianxuException("会员编号不能为空");
        if (StrUtil.isBlank(tagIdValue)) throw new QianxuException("标签id不能为空");
    ...
    public User registerPhone(String phone, Integer spreadUid) {
        ...
        if (!execute) {
            throw new QianxuException("创建用户失败!");
        }
        return user;
    }
    ...
    public void applyRegisterDefaults(User user) {
        ...
        if ("1".equals(isPromoterCfg)) {
            ...
            if (ObjectUtil.isNull(user.getPromoterTime())) {
                user.setPromoterTime(QianxuDateUtil.nowDateTime());
            }
        }
    ...
    public void saveRegisterDefaultLevelRecord(User user) {
        ...
        newLevel.setMark(StrUtil.format("尊敬的用户 {},在{}注册默认成为会员等级{}", user.getNickname(), QianxuDateUtil.nowDateTimeStr(), systemUserLevel.getName()));
        newLevel.setDiscount(systemUserLevel.getDiscount());
        newLevel.setGiveIntegral(systemUserLevel.getGiveIntegral());
        newLevel.setCreateTime(QianxuDateUtil.nowDateTime());
    ...
    public User registerByThird(RegisterThirdUserRequest thirdUserRequest) {
        User user = new User();
        user.setAccount(DigestUtils.md5Hex(QianxuUtil.getUuid() + QianxuDateUtil.getNowTime()));
...
//    }
    ...
    public void bindSpread(Integer spreadUid) {
        ...
        User user = getInfo();
        if (ObjectUtil.isNull(user)) {
            throw new QianxuException("当前用户未登录,请先登录");
        }
    ...
    public Boolean editSpread(UserUpdateSpreadRequest request) {
        ...
        if (userId.equals(spreadUid)) {
            throw new QianxuException("上级推广人不能为自己");
        }
    ...
    public Boolean updateUser(UserUpdateRequest userRequest) {
        ...
        if (ObjectUtil.isNull(tempUser)) {
            throw new QianxuException(UserResultCode.USER_NOT_EXIST);
        }
        if (tempUser.getIsLogoff()) {
            throw new QianxuException(UserResultCode.USER_LOGOFF);
        }
    ...
    public Boolean updateUserPhone(Integer id, String phone) {
        ...
        if (!matchPhone) {
            throw new QianxuException("手机号格式错误，请输入正确的手机号");
        }
        User user = getById(id);
        if (ObjectUtil.isNull(user)) {
            throw new QianxuException("对应用户不存在");
        }
        if (phone.equals(user.getPhone())) {
            throw new QianxuException("手机号与之前一致");
        }
    ...
    public Boolean updateUserPassword(Integer id, String password) {
        if (StrUtil.isBlank(password)) {
            throw new QianxuException("请输入新登录密码");
        }
        if (password.length() < 6 || password.length() > 18) {
            throw new QianxuException("密码长度需为6-18位");
        }
        User user = getById(id);
        if (ObjectUtil.isNull(user)) {
            throw new QianxuException("对应用户不存在");
        ...
        User newUser = new User();
        newUser.setUid(id);
        newUser.setPwd(QianxuUtil.encryptPassword(password, key));
        newUser.setUpdateTime(DateUtil.date());
        return userDao.updateById(newUser) > 0;
    }
    ...
    public Boolean updateUserLevel(UpdateUserLevelRequest request) {
        ...
        if (ObjectUtil.isNull(userLevel)) {
            throw new QianxuException("用户会员等级不存在，请检查用户数据");
        }
        if (userLevel.getGrade() > systemUserLevel.getGrade()) {
            if (ObjectUtil.isNull(request.getIsSub())) {
                throw new QianxuException("降低用户等级时，请选择是否扣除用户经验值");
            }
        }
        ...
        if (userLevel.getGrade() < systemUserLevel.getGrade()) {
            ...
            newLevel.setGrade(systemUserLevel.getGrade());
            newLevel.setStatus(true);
            newLevel.setMark(StrUtil.format("尊敬的用户 {},在{}管理员调整会员等级成为{}", user.getNickname(), QianxuDateUtil.nowDateTimeStr(), systemUserLevel.getName()));
            newLevel.setDiscount(systemUserLevel.getDiscount());
            newLevel.setGiveIntegral(systemUserLevel.getGiveIntegral());
            newLevel.setCreateTime(QianxuDateUtil.nowDateTime());
        ...
        if (!request.getIsSub()) {// 不扣经验
            ...
            newLevel.setGrade(systemUserLevel.getGrade());
            newLevel.setStatus(true);
            newLevel.setMark(StrUtil.format("尊敬的用户 {},在{}管理员调整会员等级成为{},不扣除经验", user.getNickname(), QianxuDateUtil.nowDateTimeStr(), systemUserLevel.getName()));
            newLevel.setDiscount(systemUserLevel.getDiscount());
            newLevel.setGiveIntegral(systemUserLevel.getGiveIntegral());
            newLevel.setCreateTime(QianxuDateUtil.nowDateTime());
            return transactionTemplate.execute(e -> {
        ...
        newLevel.setGrade(systemUserLevel.getGrade());
        newLevel.setStatus(true);
        newLevel.setMark(StrUtil.format("尊敬的用户 {},在{}管理员调整会员等级成为{},扣除经验{}", user.getNickname(), QianxuDateUtil.nowDateTimeStr(), systemUserLevel.getName(), deductionExp));
        newLevel.setDiscount(systemUserLevel.getDiscount());
        newLevel.setGiveIntegral(systemUserLevel.getGiveIntegral());
        newLevel.setCreateTime(QianxuDateUtil.nowDateTime());
        return transactionTemplate.execute(e -> {
    ...
    public Boolean logoff() {
        ...
        User user = getById(userId);
        if (ObjectUtil.isNull(user)) {
            throw new QianxuException(LoginResultCode.LOGIN_EXPIRE);
        }
        if (user.getIsLogoff()) {
            throw new QianxuException(UserResultCode.USER_LOGOFF);
        }
        ...
        if (!execute) {
            throw new QianxuException(UserResultCode.USER_LOGOFF_FAILED);
        }
