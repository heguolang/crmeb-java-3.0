...
import com.qxkj.common.constants.*;
...
import com.qxkj.common.model.cat.StoreCart;
...
import com.qxkj.common.model.product.StoreProductAttrValue;
...
public class OrderServiceImpl implements OrderService {
    ...
    public Boolean reply(StoreProductReplyAddRequest request) {
        if (StrUtil.isBlank(request.getOrderNo())) {
            throw new QianxuException("订单号参数不能为空");
        }
    ...
    public Boolean take(Integer id) {
        ...
        if (!storeOrder.getStatus().equals(Constants.ORDER_STATUS_INT_SPIKE)) {
            throw new QianxuException("订单状态错误");
        }
        if (storeOrder.getType().equals(1)) {// 视频号订单
            ...
            if (ObjectUtil.isNull(componentOrder)) {
                throw new QianxuException("没有找到视频号订单");
            }
            if (!componentOrder.getStatus().equals(30)) {
                throw new QianxuException("视频号订单不是待收货状态");
            }
            // 微信组件部分收货
            ...
            if (!recieve) {
                throw new QianxuException("自定义组件订单收货失败");
            }
    ...
    public Boolean refundApply(OrderRefundApplyRequest request) {
        ...
        if (null == existStoreOrder) throw new QianxuException("支付订单不存在");
        if (existStoreOrder.getRefundStatus() == 1) {
            throw new QianxuException("正在申请退款中");
        }

        if (existStoreOrder.getRefundStatus() == 2) {
            throw new QianxuException("订单已退款");
        }

        if (existStoreOrder.getRefundStatus() == 3) {
            throw new QianxuException("订单退款中");
        }

        existStoreOrder.setRefundStatus(1);
        existStoreOrder.setRefundReasonTime(QianxuDateUtil.nowDateTime());
        ...
        if (!execute) throw new QianxuException("申请退款失败");
        return execute;
    }
    ...
    public Boolean refundApplyTask(List<OrderRefundApplyRequest> applyList) {
        ...
        for (OrderRefundApplyRequest request : applyList) {
            ...
            storeOrder.setRefundReasonTime(QianxuDateUtil.nowDateTime());
            storeOrder.setUpdateTime(DateUtil.date());
...
//        BeanUtils.copyProperties(storeOrderCommonPage, detailPage, "list");
    ...
    public StoreOrderDetailInfoResponse detailOrder(String orderId) {
        ...
        if (ObjectUtil.isNull(storeOrder) || storeOrder.getIsDel() || storeOrder.getIsSystemDel()) {
            throw new QianxuException("订单不存在");
        }
        ...
        if (storeOrder.getType().equals(1)) {
            ...
            if (ObjectUtil.isNull(componentOrder)) {
                throw new QianxuException("视频号订单不存在");
            }
        }
    ...
    private MyRecord getOrderStatusVo(StoreOrder storeOrder) {
        ...
        if (!storeOrder.getPaid()) {
            ...
            Date timeSpace;
            timeSpace = QianxuDateUtil.addSecond(storeOrder.getCreateTime(), Double.valueOf(orderCancelTime).intValue() * 3600);
            record.set("msg", "请在" + QianxuDateUtil.dateToStr(timeSpace, Constants.DATE_FORMAT) + "前完成支付");
    ...
    public List<String> getRefundReason() {
        ...
        reasonString = QianxuUtil.UnicodeToCN(reasonString);
    ...
    public MyRecord preOrder(PreOrderRequest request) {
        if (CollUtil.isEmpty(request.getOrderDetails())) {
            throw new QianxuException("预下单订单详情列表不能为空");
        }
        ...
        // 缓存订单
        String key = user.getUid() + QianxuDateUtil.getNowTime().toString() + QianxuUtil.getUuid();
    ...
    public PreOrderResponse loadPreOrder(String preOrderNo) {
        ...
        if (!exists) {
            throw new QianxuException("预下单订单不存在");
        }
...
//        // 检测支付方式
//        if (!orderUtils.checkPayType(request.getPayType())) throw new QianxuException("暂不支持该支付方式，请刷新页面或者联系管理员");
//
//        if (request.getPayType().equals(PayConstants.PAY_TYPE_WE_CHAT)) {
//            // 检测支付渠道
//            if (StrUtil.isBlank(request.getPayChannel())) throw new QianxuException("支付渠道不能为空!");
//            if (!OrderUtils.checkPayChannel(request.getPayChannel())) throw new QianxuException("支付渠道不存在!");
//        }
        ...
        if (request.getShippingType() == 1) { // 快递配送
            if (request.getAddressId() <= 0) throw new QianxuException("请选择收货地址");
            ...
            if (ObjectUtil.isNull(userAddress) || userAddress.getIsDel()) {
                throw new QianxuException("收货地址有误");
            }
            ...
            if (StringUtils.isBlank(request.getRealName()) || StringUtils.isBlank(request.getPhone())) {
                throw new QianxuException("请填写姓名和电话");
            }
            // 自提开关是否打开
            String storeSelfMention = systemConfigService.getValueByKey(SysConfigConstants.CONFIG_KEY_STORE_SELF_MENTION);
            if (storeSelfMention.equals("false")) {
                throw new QianxuException("请先联系管理员开启门店自提");
            }
            ...
            if (ObjectUtil.isNull(systemStore) || systemStore.getIsDel() || !systemStore.getIsShow()) {
                throw new QianxuException("暂无门店无法选择门店自提");
            }
            verifyCode = QianxuUtil.randomCount(1111111111, 999999999) + "";
        ...
        if (ObjectUtil.isNotNull(orderInfoVo.getSeckillId()) && orderInfoVo.getSeckillId() > 0) {
            ...
            if (storeSeckill.getStatus().equals(0)) {
                throw new QianxuException("秒杀商品已关闭");
            }
            ...
            if (ObjectUtil.isNull(seckillAttrValue)) {
                throw new QianxuException("秒杀商品规格不存在");
            }
        ...
        // 生成订单号
        String orderNo = QianxuUtil.getOrderNo("order");

        // 购买赠送的积分：商品配置与公共设置二选一，商品优先；不支持送积分的商品不计
        int gainIntegral = 0;
        boolean hasProductIntegral = false;
        BigDecimal globalPayFee = BigDecimal.ZERO;
        List<StoreOrderInfo> storeOrderInfos = new ArrayList<>();
        for (OrderInfoDetailVo detailVo : orderInfoVo.getOrderDetailList()) {
            boolean supportGive = !Boolean.FALSE.equals(detailVo.getIsGiveIntegral());
            int lineGive = 0;
            if (supportGive && ObjectUtil.isNotNull(detailVo.getGiveIntegral()) && detailVo.getGiveIntegral() > 0) {
                lineGive = detailVo.getGiveIntegral() * detailVo.getPayNum();
                gainIntegral += lineGive;
                hasProductIntegral = true;
            } else if (supportGive) {
                // 商品未配置积分时，按行金额参与公共「消费送积分」
                BigDecimal linePrice = ObjectUtil.defaultIfNull(detailVo.getVipPrice(), detailVo.getPrice());
                if (linePrice == null) {
                    linePrice = detailVo.getPrice();
                }
                if (linePrice != null && detailVo.getPayNum() != null) {
                    globalPayFee = globalPayFee.add(linePrice.multiply(new BigDecimal(detailVo.getPayNum())));
                }
            }
            // 订单详情
            ...
            soInfo.setGiveIntegral(supportGive && ObjectUtil.isNotNull(detailVo.getGiveIntegral()) ? detailVo.getGiveIntegral() : 0);
        ...
        // 公共下单赠送积分：仅对「支持送积分且商品积分为0」的商品金额生效；有商品积分时不叠加公共
        if (!hasProductIntegral && globalPayFee.compareTo(BigDecimal.ZERO) > 0) {
            // 若整单都按公共比例，优先用实付（含优惠）；否则用未配置商品金额合计
            BigDecimal baseFee = computedOrderPriceResponse.getPayFee();
            if (baseFee == null || baseFee.compareTo(BigDecimal.ZERO) <= 0) {
                baseFee = globalPayFee;
            }
            String integralStr = systemConfigService.getValueByKey(SysConfigConstants.CONFIG_KEY_INTEGRAL_RATE_ORDER_GIVE);
            if (StrUtil.isNotBlank(integralStr) && baseFee.compareTo(BigDecimal.ZERO) > 0) {
                BigDecimal integralBig = new BigDecimal(integralStr);
                int integral = integralBig.multiply(baseFee).setScale(0, BigDecimal.ROUND_DOWN).intValue();
                if (integral > 0) {
                    gainIntegral += integral;
                }
            }
        }
        // 混合场景：部分商品有固定积分、部分走公共——公共部分按未配置商品金额计算
        if (hasProductIntegral && globalPayFee.compareTo(BigDecimal.ZERO) > 0) {
            String integralStr = systemConfigService.getValueByKey(SysConfigConstants.CONFIG_KEY_INTEGRAL_RATE_ORDER_GIVE);
            if (StrUtil.isNotBlank(integralStr)) {
                BigDecimal integralBig = new BigDecimal(integralStr);
                int integral = integralBig.multiply(globalPayFee).setScale(0, BigDecimal.ROUND_DOWN).intValue();
                if (integral > 0) {
                    gainIntegral += integral;
                }
            }
...
//        storeOrder.setPayType(request.getPayType());
        ...
        storeOrder.setCreateTime(QianxuDateUtil.nowDateTime());
...
//        storeOrder.setIsChannel(isChannel);
        ...
        if (!execute) {
            throw new QianxuException("订单生成失败");
        }
    ...
    private List<MyRecord> validateProductStock(OrderInfoVo orderInfoVo, User user) {
        ...
        if (orderInfoVo.getBargainId() > 0) {
            ...
            if (storeBargain.getStock().equals(0) || detailVo.getPayNum() > storeBargain.getStock()) {
                throw new QianxuException("砍价商品库存不足");
            }
            ...
            if (ObjectUtil.isNull(bargainAttrValue)) {
                throw new QianxuException("砍价商品规格不存在");
            }
        ...
        if (orderInfoVo.getCombinationId() > 0) {
            ...
            if (ObjectUtil.isNull(combinationAttrValue)) {
                throw new QianxuException("拼团商品规格不存在");
            }
        ...
        if (orderInfoVo.getIsVideo()) {// 视频号订单
            ...
            if (ObjectUtil.isNull(product)) {
                throw new QianxuException("商品信息不存在，请刷新后重新选择");
            }
            if (product.getIsDel()) {
                throw new QianxuException("商品已删除，请刷新后重新选择");
            }
            if (!product.getStatus().equals(5)) {
                throw new QianxuException("商品已下架，请刷新后重新选择");
            }
            if (product.getStock().equals(0) || product.getStock() < detailVo.getPayNum()) {
                throw new QianxuException("商品库存不足，请刷新后重新选择");
            }
            // 查询商品规格属性值信息
            ...
            if (ObjectUtil.isNull(attrValue)) {
                throw new QianxuException("商品规格信息不存在，请刷新后重新选择");
            }
            if (attrValue.getStock().equals(0) || attrValue.getStock() < detailVo.getPayNum()) {
                throw new QianxuException("商品规格库存不足，请刷新后重新选择");
            }
            // 查询视频商品规格属性值
            ...
            if (ObjectUtil.isNull(productSku)) {
                throw new QianxuException("商品sku信息不存在，请刷新后重新选择");
            }
            if (productSku.getStockNum().equals(0) || productSku.getStockNum() < detailVo.getPayNum()) {
                throw new QianxuException("商品sku库存不足，请刷新后重新选择");
            }
        ...
        orderDetailList.forEach(e -> {
            ...
            if (ObjectUtil.isNull(storeProduct)) {
                throw new QianxuException("购买的商品信息不存在");
            }
            if (storeProduct.getIsDel()) {
                throw new QianxuException("购买的商品已删除");
            }
            if (!storeProduct.getIsShow()) {
                throw new QianxuException("购买的商品已下架");
            }
            if (storeProduct.getStock().equals(0) || e.getPayNum() > storeProduct.getStock()) {
                throw new QianxuException("购买的商品库存不足");
            }
            // 查询商品规格属性值信息
            ...
            if (ObjectUtil.isNull(attrValue)) {
                throw new QianxuException("购买的商品规格信息不存在");
            }
        ...
        if (request.getPreOrderType().equals("buyNow")) {// 立即购买
            ...
            if (detailRequest.getSeckillId() > 0) {// 秒杀
                ...
                if (!attrValue.getIsShow()) {
                    throw new QianxuException("商品规格信息无效，请刷新后重新选择");
                }
                ...
                // 是否支持送积分：默认支持；关闭则 giveIntegral 记 0
                boolean supportGive = !Boolean.FALSE.equals(storeProduct.getIsGiveIntegral());
                detailVo.setIsGiveIntegral(supportGive);
                detailVo.setGiveIntegral(supportGive ? storeProduct.getGiveIntegral() : 0);
                detailVo.setIntegralDeduct(ObjectUtil.defaultIfNull(storeProduct.getIntegralDeduct(), 0));
                detailVo.setIsIntegralDeductBrokerage(!Boolean.FALSE.equals(storeProduct.getIsIntegralDeductBrokerage()));
    ...
    private OrderInfoDetailVo validatePreOrderSeckill(PreOrderDetailRequest detailRequest, User user) {
        ...
        if (!seckillAttrValue.getIsShow()) {
            throw new QianxuException("商品规格信息无效");
        }
    ...
    private MyRecord commonValidateSeckill(StoreSeckill storeSeckill, StoreProductAttrValue seckillAttrValue, User user, Integer productNum) {
        ...
        if (ObjectUtil.isNull(productAttrValue)) {
            throw new QianxuException("秒杀主商品规格不存在");
        }
        if (productAttrValue.getStock() <= 0 || productNum > productAttrValue.getStock()) {
            throw new QianxuException("秒杀主商品规格库存不足");
        }
        ...
        if (ObjectUtil.isNull(seckillManger)) {
            throw new QianxuException("秒杀时段不存在");
        }
        // 判断日期是否过期
        DateTime nowDateTime = cn.hutool.core.date.DateUtil.date();
        String stopTimeStr = QianxuDateUtil.dateToStr(storeSeckill.getStopTime(), Constants.DATE_FORMAT_DATE);
        Date stopDate = QianxuDateUtil.strToDate(stopTimeStr + " " + seckillManger.getEndTime() + ":00:00", Constants.DATE_FORMAT);
        if (nowDateTime.getTime() - stopDate.getTime() >= 0) {
            throw new QianxuException("秒杀商品已过期");
        }
        ...
        if (CollUtil.isNotEmpty(userCurrentDaySecKillOrders)) {
            ...
            if (unPayOrders.size() > 0) {
                throw new QianxuException("您有秒杀待支付订单，请支付后再购买");
            }

            // 判断是否达到上限
            if (userCurrentDaySecKillOrders.size() >= storeSeckill.getNum()) {
                throw new QianxuException("您已经达到当前秒杀活动上限");
            }
        }
    ...
    private OrderInfoDetailVo validatePreOrderBargain(PreOrderDetailRequest detailRequest, User user) {
        if (detailRequest.getBargainUserId() <= 0) {
            throw new QianxuException("用户砍价活动id必须大于0");
        }
        // 砍价部分判断
    ...
    private MyRecord commonValidateBargain(StoreBargain storeBargain, StoreProductAttrValue bargainAttrValue, Integer bargainUserId, User user, Integer productNum) {
        ...
        if (!bargainUser.getStatus().equals(3)) {
            if (bargainUser.getBargainPriceMin().compareTo(bargainUser.getBargainPrice().subtract(bargainUser.getPrice())) != 0) {
                throw new QianxuException("请先完成砍价活动");
            }
        }
        // 判断是否有订单
        ...
        if (ObjectUtil.isNotNull(bargainOrder)) {
            if (!bargainOrder.getPaid()) {
                throw new QianxuException("订单已创建，尚未支付");
            }
            throw new QianxuException("该砍价活动已创建了订单");
        }
        ...
        if (ObjectUtil.isNull(productAttrValue)) {
            throw new QianxuException("砍价主商品规格不存在");
        }
        if (productAttrValue.getStock() <= 0 || productNum > productAttrValue.getStock()) {
            throw new QianxuException("砍价主商品规格库存不足");
        }
    ...
    private OrderInfoDetailVo validatePreOrderCombination(PreOrderDetailRequest detailRequest, User user) {
        ...
        if (storeCombination.getStock().equals(0) || detailRequest.getProductNum() > storeCombination.getStock()) {
            throw new QianxuException("拼团商品库存不足");
        }
    ...
    private MyRecord commonValidateCombination(StoreCombination storeCombination, StoreProductAttrValue combinationAttrValue, User user, Integer productNum) {
        ...
        if (ObjectUtil.isNull(productAttrValue)) {
            throw new QianxuException("拼团主商品规格不存在");
        }
        if (productAttrValue.getStock() <= 0 || productNum > productAttrValue.getStock()) {
            throw new QianxuException("拼团主商品规格库存不足");
        }
    ...
    private List<OrderInfoDetailVo> validatePreOrderAgain(PreOrderDetailRequest detailRequest, User user) {
        ...
        if (StrUtil.isBlank(detailRequest.getOrderNo())) {
            throw new QianxuException("再次购买订单编号不能为空");
        }
        ...
        infoVoList.forEach(e -> {
            ...
            boolean supportGive = !Boolean.FALSE.equals(storeProduct.getIsGiveIntegral());
            tempDetailVo.setIsGiveIntegral(supportGive);
            tempDetailVo.setGiveIntegral(supportGive ? storeProduct.getGiveIntegral() : 0);
            tempDetailVo.setIntegralDeduct(ObjectUtil.defaultIfNull(storeProduct.getIntegralDeduct(), 0));
            tempDetailVo.setIsIntegralDeductBrokerage(!Boolean.FALSE.equals(storeProduct.getIsIntegralDeductBrokerage()));
    ...
    private ComputedOrderPriceResponse computedPrice(OrderComputedPriceRequest request, OrderInfoVo orderInfoVo, User user) {
        ...
        if (request.getShippingType().equals(2)) {// 到店自提，不计算运费
            ...
            if (ObjectUtil.isNull(userAddress)) {
                throw new QianxuException("用户地址不存在");
            } else {
                getFreightFee(orderInfoVo, userAddress);
        ...
        if (ObjectUtil.isNull(request.getCouponId()) || request.getCouponId() <= 0) {
            ...
            throw new QianxuException("营销活动商品无法使用优惠券");
        } else if (orderInfoVo.getIsVideo()) {
            throw new QianxuException("视频号商品无法使用优惠券");
        } else {
            // 判断优惠券是否可以使用
            ...
            if (ObjectUtil.isNull(storeCouponUser) || !storeCouponUser.getUid().equals(user.getUid())) {
                throw new QianxuException("优惠券领取记录不存在！");
            }
            if (storeCouponUser.getStatus() == 1) {
                throw new QianxuException("此优惠券已使用！");
            }

            if (storeCouponUser.getStatus() == 2) {
                throw new QianxuException("此优惠券已失效！");
            }
            //判断是否在使用时间内
            Date date = QianxuDateUtil.nowDateTime();
            if (storeCouponUser.getStartTime().compareTo(date) > 0) {
                throw new QianxuException("此优惠券还未到达使用时间范围之内！");
            }
            if (date.compareTo(storeCouponUser.getEndTime()) > 0) {
                throw new QianxuException("此优惠券已经失效了");
            }
            if (storeCouponUser.getMinPrice().compareTo(orderInfoVo.getProTotalFee()) > 0) {
                throw new QianxuException("总金额小于优惠券最小使用金额");
            }
            //检测优惠券信息
            if (storeCouponUser.getUseType().equals(1)) {
                ...
                if (CollUtil.isEmpty(productIdList)) {
                    throw new QianxuException("没有找到商品");
                }
                //设置优惠券所提供的集合
                List<Integer> primaryKeyIdList = QianxuUtil.stringToArray(storeCouponUser.getPrimaryKey());
                ...
                if (storeCouponUser.getUseType() == 2) {
                    ...
                    if (CollUtil.isEmpty(primaryKeyIdList)) {
                        throw new QianxuException("此优惠券为商品券，请购买相关商品之后再使用！");
                    }
                ...
                if (storeCouponUser.getUseType() == 3) {
                    ...
                    if (!categoryIdList.contains(primaryKey)) {
                        throw new QianxuException("此优惠券为分类券，请购买相关分类下的商品之后再使用！");
                    }
                    ...
                    for (StoreProduct product : storeProductList) {
                        List<Integer> cateIdList = QianxuUtil.stringToArray(product.getCateId());
        ...
        // 积分部分：按商品「最多可用积分抵扣」分摊，0 表示该商品不支持抵扣
        ...
        priceResponse.setProTotalFee(orderInfoVo.getProTotalFee());
        // 先清空行上历史抵扣快照
        if (CollUtil.isNotEmpty(orderInfoVo.getOrderDetailList())) {
            for (OrderInfoDetailVo d : orderInfoVo.getOrderDetailList()) {
                d.setLineUseIntegral(0);
                d.setLineDeductionPrice(BigDecimal.ZERO);
            }
        }
        ...
        String integralRatioStr = systemConfigService.getValueByKey(SysConfigConstants.CONFIG_KEY_INTEGRAL_RATE);
        if (StrUtil.isBlank(integralRatioStr) || new BigDecimal(integralRatioStr).compareTo(BigDecimal.ZERO) <= 0
                || CollUtil.isEmpty(orderInfoVo.getOrderDetailList()) || payPrice.compareTo(BigDecimal.ZERO) <= 0) {
            priceResponse.setDeductionPrice(BigDecimal.ZERO);
            priceResponse.setSurplusIntegral(user.getIntegral());
            priceResponse.setPayFee(payPrice.add(priceResponse.getFreightFee()));
            priceResponse.setUsedIntegral(0);
        ...
        BigDecimal integralRatio = new BigDecimal(integralRatioStr);

        // 是否有任一商品配置了抵扣上限（>0）。全为 0 时兼容旧逻辑：整单按用户积分抵扣
        boolean anyProductCap = orderInfoVo.getOrderDetailList().stream()
                .anyMatch(d -> ObjectUtil.defaultIfNull(d.getIntegralDeduct(), 0) > 0);
        if (!anyProductCap) {
            BigDecimal deductionPrice = new BigDecimal(user.getIntegral()).multiply(integralRatio);
            if (deductionPrice.compareTo(payPrice) < 0) {
                ...
                if (payPrice.compareTo(BigDecimal.ZERO) > 0) {
                    int usedIntegral = payPrice.divide(integralRatio, 0, BigDecimal.ROUND_UP).intValue();
            ...
            // 整单抵扣时按行金额占比分摊到明细，供「抵扣不参与分佣」使用
            allocateLineDeductionByAmount(orderInfoVo.getOrderDetailList(), priceResponse.getUsedIntegral(), deductionPrice);
            priceResponse.setPayFee(payPrice.add(priceResponse.getFreightFee()));
            priceResponse.setDeductionPrice(deductionPrice);
            return priceResponse;
        }

        int remainIntegral = user.getIntegral();
        BigDecimal remainPay = payPrice;
        int totalUsedIntegral = 0;
        BigDecimal totalDeduction = BigDecimal.ZERO;
        for (OrderInfoDetailVo detail : orderInfoVo.getOrderDetailList()) {
            int perUnitMax = ObjectUtil.defaultIfNull(detail.getIntegralDeduct(), 0);
            int payNum = Math.max(ObjectUtil.defaultIfNull(detail.getPayNum(), 1), 1);
            int lineMaxIntegral = perUnitMax > 0 ? perUnitMax * payNum : 0;
            if (lineMaxIntegral <= 0 || remainIntegral <= 0 || remainPay.compareTo(BigDecimal.ZERO) <= 0) {
                detail.setLineUseIntegral(0);
                detail.setLineDeductionPrice(BigDecimal.ZERO);
                continue;
            }
            int canUse = Math.min(lineMaxIntegral, remainIntegral);
            BigDecimal canMoney = new BigDecimal(canUse).multiply(integralRatio).setScale(2, BigDecimal.ROUND_DOWN);
            if (canMoney.compareTo(remainPay) > 0) {
                canMoney = remainPay;
                canUse = canMoney.divide(integralRatio, 0, BigDecimal.ROUND_UP).intValue();
                if (canUse > lineMaxIntegral) {
                    canUse = lineMaxIntegral;
                }
                if (canUse > remainIntegral) {
                    canUse = remainIntegral;
                }
                canMoney = new BigDecimal(canUse).multiply(integralRatio).setScale(2, BigDecimal.ROUND_DOWN);
                if (canMoney.compareTo(remainPay) > 0) {
                    canMoney = remainPay;
                }
            }
            if (canUse <= 0 || canMoney.compareTo(BigDecimal.ZERO) <= 0) {
                detail.setLineUseIntegral(0);
                detail.setLineDeductionPrice(BigDecimal.ZERO);
                continue;
            }
            detail.setLineUseIntegral(canUse);
            detail.setLineDeductionPrice(canMoney);
            remainIntegral -= canUse;
            remainPay = remainPay.subtract(canMoney);
            totalUsedIntegral += canUse;
            totalDeduction = totalDeduction.add(canMoney);
        }
        if (totalUsedIntegral <= 0) {
            priceResponse.setDeductionPrice(BigDecimal.ZERO);
            priceResponse.setSurplusIntegral(user.getIntegral());
            priceResponse.setPayFee(payPrice.add(priceResponse.getFreightFee()));
            priceResponse.setUsedIntegral(0);
        ...
        priceResponse.setUsedIntegral(totalUsedIntegral);
        priceResponse.setSurplusIntegral(user.getIntegral() - totalUsedIntegral);
        priceResponse.setDeductionPrice(totalDeduction);
        priceResponse.setPayFee(remainPay.add(priceResponse.getFreightFee()));
        return priceResponse;
    }

    /**
     * 整单积分抵扣时，按行实付占比把抵扣积分/金额写入明细快照。
     */
    private void allocateLineDeductionByAmount(List<OrderInfoDetailVo> details, int totalUsedIntegral, BigDecimal totalDeduction) {
        if (CollUtil.isEmpty(details) || totalUsedIntegral <= 0 || totalDeduction == null
                || totalDeduction.compareTo(BigDecimal.ZERO) <= 0) {
            return;
        }
        BigDecimal amountSum = BigDecimal.ZERO;
        for (OrderInfoDetailVo d : details) {
            BigDecimal unit = ObjectUtil.defaultIfNull(d.getVipPrice(), d.getPrice());
            int num = Math.max(ObjectUtil.defaultIfNull(d.getPayNum(), 1), 1);
            if (unit != null) {
                amountSum = amountSum.add(unit.multiply(new BigDecimal(num)));
            }
        }
        if (amountSum.compareTo(BigDecimal.ZERO) <= 0) {
            return;
        }
        int assignedIntegral = 0;
        BigDecimal assignedMoney = BigDecimal.ZERO;
        for (int i = 0; i < details.size(); i++) {
            OrderInfoDetailVo d = details.get(i);
            BigDecimal unit = ObjectUtil.defaultIfNull(d.getVipPrice(), d.getPrice());
            int num = Math.max(ObjectUtil.defaultIfNull(d.getPayNum(), 1), 1);
            BigDecimal lineAmount = unit == null ? BigDecimal.ZERO : unit.multiply(new BigDecimal(num));
            if (i == details.size() - 1) {
                d.setLineUseIntegral(Math.max(totalUsedIntegral - assignedIntegral, 0));
                d.setLineDeductionPrice(totalDeduction.subtract(assignedMoney).max(BigDecimal.ZERO));
            } else {
                BigDecimal ratio = lineAmount.divide(amountSum, 6, BigDecimal.ROUND_HALF_UP);
                int lineIntegral = ratio.multiply(new BigDecimal(totalUsedIntegral)).setScale(0, BigDecimal.ROUND_DOWN).intValue();
                BigDecimal lineMoney = totalDeduction.multiply(ratio).setScale(2, BigDecimal.ROUND_DOWN);
                d.setLineUseIntegral(lineIntegral);
                d.setLineDeductionPrice(lineMoney);
                assignedIntegral += lineIntegral;
                assignedMoney = assignedMoney.add(lineMoney);
            }
        }
    }
}
