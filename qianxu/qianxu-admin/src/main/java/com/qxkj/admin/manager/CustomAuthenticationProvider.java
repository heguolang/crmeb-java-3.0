package com.qxkj.admin.manager;
...
//    private final UserDetailServiceImpl userDetailsService = new UserDetailServiceImpl();
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        if (ObjectUtil.isNull(userDetails)) {
            throw new QianxuException("用户名不存在");
        }
        // base64加密获取真正密码
        String encryptPassword = QianxuUtil.encryptPassword(password, name);
        if (!userDetails.getUser().getPwd().equals(encryptPassword)) {
            throw new QianxuException("账号或者密码不正确");
        }
