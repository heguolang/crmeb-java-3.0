...
public class SystemStatusServiceImpl implements SystemStatusService {
    ...
    public static CpuInfo getCpuInfo() {
        ...
        double userUtilization = QianxuUtil.round(user / totalTime);
        double systemUtilization = QianxuUtil.round( system / totalTime);
        double idleRate = QianxuUtil.round(idle / totalTime);
    ...
    public static systemMemoryInfo getMemoryInfo(){
        ...
        // 格式化易读单位
        String totalMemory = QianxuUtil.formatBytes(totalBytes);
        String freeMemory = QianxuUtil.formatBytes(availableBytes);
        String usedMemory = QianxuUtil.formatBytes(totalBytes - availableBytes);
        double systemUsageRate = QianxuUtil.round(usageRatio);
    ...
    private static JvmMemoryInfo getJvmMemoryInfo() {
        ...
        // 格式化字符串
        String heapTotal = QianxuUtil.formatBytes(heapCommittedBytes);
        String heapUsed = QianxuUtil.formatBytes(heapUsedBytes);
        String heapFree = QianxuUtil.formatBytes(heapCommittedBytes - heapUsedBytes);
        double jvmUsageRate = QianxuUtil.round(heapUsageRatio);
    ...
    private static ServerInfo getServerInfo(){
        ...
        String serverIP = QianxuUtil.getPrimaryIpAddress();
    ...
    private static JvmInfo getJvmInfo(){
        ...
        String uptime = QianxuUtil.formatUptime(uptimeMillis);
        ...
        try {
            ...
            fileStores.stream()
                    ...
                    .forEach(fs -> {
                        ...
                        diskInfo.setTotalSize(QianxuUtil.formatBytes(totalBytes));
                        diskInfo.setFreeSize(QianxuUtil.formatBytes(freeBytes));
                        diskInfo.setUsedSize(QianxuUtil.formatBytes(totalBytes - freeBytes));
                        diskInfo.setUsedPercentage(QianxuUtil.round(usedPercentageRate));
