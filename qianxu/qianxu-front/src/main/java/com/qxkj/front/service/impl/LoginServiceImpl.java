...
public class LoginServiceImpl implements LoginService {
    ...
    public LoginResponse login(LoginRequest loginRequest) {
        ...
        if (ObjectUtil.isNull(user)) {
            throw new QianxuException("此账号未注册");
        }
        if (!user.getStatus()) {
            throw new QianxuException("此账号被禁用");
        }

        // 校验密码
        String password = QianxuUtil.encryptPassword(loginRequest.getPassword(), loginRequest.getPhone());
        if (!user.getPwd().equals(password)) {
            throw new QianxuException("密码错误");
        }
    ...
    public LoginResponse register(LoginRequest loginRequest) {
        ...
        if (spreadPid > 0) {
            ...
            if (ObjectUtil.isNull(spreadUser) || !Boolean.TRUE.equals(spreadUser.getStatus())) {
                throw new QianxuException("推荐人ID不存在或已禁用");
            }
            // 预检绑定条件，避免注册成功却静默未绑定
            User probe = new User();
            probe.setUid(null);
            ...
            if (!userService.checkBingSpread(probe, spreadPid, "new")) {
                ...
                if (StrUtil.isBlank(isOpen) || "0".equals(isOpen)) {
                    throw new QianxuException("分销功能未开启，无法绑定推荐人，请先在后台开启分销");
                }
                ...
                if (!"2".equals(brokerageModel) && !Boolean.TRUE.equals(spreadUser.getIsPromoter())) {
                    throw new QianxuException("推荐人不是推广员，当前为指定分销模式，无法绑定");
                }
                throw new QianxuException("推荐人绑定失败，请检查推荐人ID或分销设置");
            }
        }
        ...
        if (ObjectUtil.isNull(user)) {
            throw new QianxuException("注册失败，请稍后重试");
        }

        // 覆盖为自定义密码（registerPhone 默认用的是手机号加密）
        user.setPwd(QianxuUtil.encryptPassword(password, phone));
        user.setUpdateTime(DateUtil.date());
