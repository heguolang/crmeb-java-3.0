        ...
        switch (request.getExtractType()) {
            case "weixin":
                if (StrUtil.isBlank(request.getWechat())) {
                    throw new QianxuException("请填写微信号！");
                }
            ...
            case "alipay":
                if (StrUtil.isBlank(request.getAlipayCode())) {
                    throw new QianxuException("请填写支付宝账号！");
                }
            ...
            case "bank":
                if (StrUtil.isBlank(request.getBankName())) {
                    throw new QianxuException("请填写银行名称！");
                }
                if (StrUtil.isBlank(request.getBankCode())) {
                    throw new QianxuException("请填写银行卡号！");
                }
                request.setWechat(null);
                request.setAlipayCode(null);
                break;
            default:
                throw new QianxuException("请选择支付方式");
        }
...
//        });
    ...
    public UserRechargeFrontResponse getRechargeConfig() {
        ...
        if (StrUtil.isNotBlank(rechargeAttention)) {
            rechargeAttentionList = QianxuUtil.stringToArrayStrRegex(rechargeAttention, "\n");
        }
...
//        BigDecimal orderStatusSum = storeOrderService.getSumBigDecimal(info.getUid(), null);
    ...
    public UserSpreadOrderResponse getSpreadOrder(PageParamRequest pageParamRequest) {
        ...
        if (ObjectUtil.isNull(user)) {
            throw new QianxuException("用户数据异常");
        }
     ...
     */
    @Override
    @Transactional(rollbackFor = {RuntimeException.class, Error.class, QianxuException.class})
    public OrderPayResultResponse recharge(UserRechargeRequest request) {
        ...
        if (compareResult > 0) {
            throw new QianxuException("充值金额不能低于" + rechargeMinAmountStr);
        }
        ...
        if (request.getGroupDataId() > 0) {
            ...
            if (ObjectUtil.isNull(systemGroupData)) {
                throw new QianxuException("您选择的充值方式已下架");
            }
            //售价和赠送
        ...
        userRecharge.setOrderId(QianxuUtil.getOrderNo("recharge"));
        ...
        if (!save) {
            throw new QianxuException("生成充值订单失败!");
        }
        return response;
    }
    ...
    public Boolean transferIn(BigDecimal price) {
        if (price.compareTo(BigDecimal.ZERO) <= 0) {
            throw new QianxuException("转入金额不能为0");
        }
        //当前可提现佣金
        User user = userService.getInfo();
        if (ObjectUtil.isNull(user)) {
        ...
        if (subtract.compareTo(price) < 0) {
            throw new QianxuException("您当前可充值余额为 " + subtract + "元");
        }
        // userBill现金增加记录
    ...
    public CommonPage<UserRechargeBillRecordResponse> nowMoneyBillRecord(String type, PageParamRequest pageRequest) {
        ...
        list.forEach(i -> {
            String month = StrUtil.subPre(QianxuDateUtil.dateToStr(i.getCreateTime(), Constants.DATE_FORMAT), 7);
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            if (finalIsNew) {// 新用户
                ...
                if (userService.checkBingSpread(finalUser, registerThirdUserRequest.getSpreadPid(), "new")) {
                    ...
                    finalUser.setSpreadTime(QianxuDateUtil.nowDateTime());
        ...
        if (request.getType().equals("public") || request.getType().equals("iosWx") || request.getType().equals("androidWx")) {
            ...
            if (StrUtil.isNotBlank(request.getCaptcha())) {
                if (StrUtil.isBlank(request.getPhone())) {
                    throw new QianxuException("手机号不能为空");
                }
                boolean matchPhone = ReUtil.isMatch(RegularConstants.PHONE_TWO, request.getPhone());
                if (!matchPhone) {
                    throw new QianxuException("手机号格式错误，请输入正确得手机号");
                ...
                if (StrUtil.isBlank(request.getCode())) {
                    throw new QianxuException("小程序获取手机号code不能为空");
                }
                if (StrUtil.isBlank(request.getEncryptedData())) {
                    throw new QianxuException("小程序获取手机号加密数据不能为空");
                }
                if (StrUtil.isBlank(request.getIv())) {
                    throw new QianxuException("小程序获取手机号加密算法的初始向量不能为空");
                }
                // 获取appid
