# QIANXU Java 3.0 本地部署文档（Windows）

> ⭐ **部署后数据库只需执行这一个脚本**：`qianxu/sql/oneclick/ALL_IN_ONE.sql`
> `mysql -uroot -p密码 --default-character-set=utf8mb4 < ALL_IN_ONE.sql`（建库+全量结构数据+全量补丁）
> 已有生产库只补功能时改跑 `qianxu/sql/oneclick/02_patches_all.sql`（幂等，不清数据）。

> 部署日期：2026-09-17　部署目录：`D:\qianxu-java-3.0`　代码来源：`https://github.com/heguolang/qianxu-java-3.0.git`（master @ abe4b36）
...
- 切换前备份：MariaDB 全量 `D:\env\backup\qianxu_mariadb_20260918.sql`（131 表）、升级 8.0 前 MySQL 5.7 全量 `D:\env\backup\qianxu_mysql57_20260918_before8.sql`（131 表，导入 8.0 无报错）。
- MariaDB 服务 `QIANXU-DB` 已停止并改为**手动启动**（数据目录 `D:\env\mariadb-10.6.28-winx64\data` 原样保留，作为回退手段；如需回退：停 MySQL80 → 启动 QIANXU-DB，两者都占用 3306，不要同时开）。
- 5.7.38 已按要求清除：服务已 remove、目录与安装包已删；客户端用 `D:\env\mysql-8.0.29-winx64\bin\mysql.exe`。
- **MariaDB 专用兼容脚本已作废并删除**：`qianxu/sql/mariadb_any_value_compat.sql`（给 MariaDB 造一个 `ANY_VALUE()` 恒等函数）在 MySQL 5.7 上**有害** —— 用户函数会遮蔽内置函数，反而偏离生产行为。该文件已从仓库删除，`qianxu/sql/oneclick/02_patches_all.sql` 里对应的补丁段也已移除。库中残留的同名函数已 `DROP FUNCTION IF EXISTS ANY_VALUE`，实测 `information_schema.ROUTINES` 中 qianxu 库 0 条自定义函数，`SELECT ANY_VALUE(id) ... GROUP BY uid` 走内置函数正常。
...
```
D:\qianxu-java-3.0
├─ qianxu/                    后端（Spring Boot 2.2.6 + MyBatis-Plus 3.3.1）
│  ├─ qianxu-admin/           后台管理 API（:8080）→ target/Qianxu-admin.jar
│  ├─ qianxu-front/           会员端 API（:8081）   → target/Qianxu-front.jar
│  ├─ qianxu-common/
│  ├─ qianxu-service/
│  ├─ sql/                   建表与补丁脚本
│  └─ qianxuimage/            上传图片资源（531 个文件）
...
curl -L -o D:\qianxu-master.zip https://codeload.github.com/heguolang/qianxu-java-3.0/zip/refs/heads/master
tar -xf D:\qianxu-master.zip -C D:\
mv D:\qianxu-java-3.0-master D:\qianxu-java-3.0
```

### 2. 初始化数据库（按顺序，前 13 步为建表/补丁，第 14 步为业务数据）

```bash
set MYSQL=D:\env\mysql-8.0.29-winx64\bin\mysql.exe -uroot -p123456 --default-character-set=utf8mb4 qianxu
%MYSQL% < qianxu\sql\Qianxu_v3.0.sql
%MYSQL% < qianxu\sql\add_missing_team_level_tables.sql
%MYSQL% < qianxu\sql\add_missing_columns.sql
%MYSQL% < qianxu\sql\upgrade_team_level_direct.sql
%MYSQL% < qianxu\sql\add_login_notice_config.sql
%MYSQL% < qianxu\sql\add_user_update_password_menu.sql
%MYSQL% < qianxu\sql\add_admin_log_menu_and_table.sql
%MYSQL% < qianxu\sql\reset_default_category.sql
%MYSQL% < qianxu\sql\update_copyright_company_name.sql
%MYSQL% < qianxu\sql\agent.sql
%MYSQL% < qianxu\sql\stock.sql
%MYSQL% < qianxu\sql\update_deploy_config.sql
%MYSQL% < qianxu\sql\export_admin_settings.sql
%MYSQL% < db-data\qianxu_full_data_export.sql
```
...
导入后：**131 张表**、用户 10 条、后台设置/菜单/站点配置均完整。生产/空库环境可直接用一键脚本 `qianxu\sql\oneclick\deploy.bat`（生产同款，其合并补丁里对应的 MariaDB 兼容段已同步移除）。

### 3. 图片资源

```bash
xcopy /E /Y /I db-data\qianxu_image\qianxuimage\*  qianxu\qianxuimage\
```

对应 `application.yml` 的 `qianxu.imagePath: D:/qianxu-java-3.0/qianxu/`（注意斜杠结尾）。

### 4. 配置修改（已改好，仅列出差异）

`qianxu/qianxu-admin/src/main/resources/application.yml` 与 `qianxu/qianxu-front/src/main/resources/application.yml`：

| 配置项 | 原值 | 现值 |
|---|---|---|
| `qianxu.imagePath` | `/Users/qianxu/.../qianxu/`（macOS 路径） | `D:/qianxu-java-3.0/qianxu/` |
| `spring.datasource.password` | `root` | `123456` |
| `qianxu.demoSite` | `true`（手机号掩码成 139\*\*\*\*0004） | `false`（显示完整手机号，两个模块均已改） |

> `demoSite` 仅影响手机号掩码。当前用启动参数 `--qianxu.demoSite=false` 覆盖（`start-windows.bat` 内置），源码配置也已改为 false，重新打包后自动生效。若要对访客隐藏完整手机号，把该值改回 `true` 重启即可。
...
cd D:\qianxu-java-3.0\qianxu
D:\env\apache-maven-3.9.16\bin\mvn.cmd package -DskipTests -Dmaven.repo.local=D:/env/maven-repo

java -Dfile.encoding=UTF-8 -Dsun.jnu.encoding=UTF-8 -jar qianxu\qianxu-admin\target\Qianxu-admin.jar --server.port=8080
java -Dfile.encoding=UTF-8 -Dsun.jnu.encoding=UTF-8 -jar qianxu\qianxu-front\target\Qianxu-front.jar --server.port=8081
```
...
```bash
E:\HBuilderX\HBuilderX\cli.exe project open --path D:\qianxu-java-3.0\app
E:\HBuilderX\HBuilderX\cli.exe launch web --project D:\qianxu-java-3.0\app --browser Chrome
```
...
```bash
D:\qianxu-java-3.0\local-dev\start-windows.bat     # 依次拉起 MySQL80(服务)/Redis/8080/8081/9527
D:\qianxu-java-3.0\local-dev\stop-windows.bat      # 停止两个 jar + 前端
D:\qianxu-java-3.0\local-dev\stop-windows.bat full # 连带停止 Redis 与 MySQL80 服务（net stop，需管理员）
```
...
4. **Redis 需要密码**：项目配置 `password: 123456`，启动时加 `--requirepass 123456`；早期用过的 `redis-lite`（Node 实现）不支持 Lua 脚本，QIANXU 不可用，已替换为真实 Redis。
...
自定义开发的批发订货模块，代码位于 `qianxu/qianxu-*` 的 `com.qxkj.*.stock` 包，管理后台入口"订货管理"，会员端接口前缀 `api/front/stock`。
...
- 初始化/迁移 SQL：`qianxu/sql/stock_upgrade_conditions.sql`（重建库后需重新执行）。
...
- 迁移 SQL：`qianxu/sql/stock_changelog_product_rel.sql`（重建库后需重新执行）。
...
- 结算周期由系统配置 `stock_ladder_cycle` 决定（1=月度 / 2=季度 / 3=年度）；定时任务 `StockLadderSettleTask`（qianxu-admin）在每月 1 日 01:00 自动结算上一周期，也可在后台奖励规则页手动指定周期与月份结算（接口 `/admin/stock/reward/monthlySettle`，参数 type + month）。
- 初始化/迁移 SQL：`qianxu/sql/stock_ladder_period_reward.sql`（重建库后需重新执行）。
...
- 会员端奖金中心不再有独立提现入口/提现记录，「申请提现」按钮直接跳转系统统一佣金提现页 `/pages/users/user_cash/index`，走 QIANXU 原生佣金提现（user_extract）逻辑。
...
```bash
D:\env\mysql-8.0.29-winx64\bin\mysqldump.exe -uroot -p123456 --default-character-set=utf8mb4 --single-transaction --routines --triggers --events --databases qianxu > D:\env\backup\qianxu_backup.sql
```

> 历史备份均可回退：MariaDB 全量 `D:\env\backup\qianxu_mariadb_20260918.sql`、MySQL 5.7 全量 `D:\env\backup\qianxu_mysql57_20260918_before8.sql`；MariaDB 服务 `QIANXU-DB` 与数据目录 `D:\env\mariadb-10.6.28-winx64\data` 原样保留。

图片一并备份 `qianxu\qianxuimage\` 目录。`db-data\` 目录是上一台机器（macOS）导出的完整数据包，含导入顺序说明，可作为还原参考。
