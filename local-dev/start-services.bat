...
if errorlevel 1 (
  start "qianxu-redis" /B "D:\env\redis\redis-server.exe" --port 6379 --requirepass 123456 --maxmemory 512mb
  echo       started
...
if errorlevel 1 (
  start "qianxu-admin" /B "%JAVA_HOME%\bin\java.exe" %JAVA_OPTS% -jar "%PROJ%\qianxu\qianxu-admin\target\Qianxu-admin.jar" --server.port=8080 %DEMO_FLAG%
  echo       starting
) else ( echo       already running )
...
if errorlevel 1 (
  start "qianxu-front" /B "%JAVA_HOME%\bin\java.exe" %JAVA_OPTS% -jar "%PROJ%\qianxu\qianxu-front\target\Qianxu-front.jar" --server.port=8081 %DEMO_FLAG%
  echo       starting
) else ( echo       already running )
