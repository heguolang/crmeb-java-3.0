#!/usr/bin/env bash
# ============================================================
# CRMEB Java 3.0 一键导入数据库（Linux / 宝塔）
# 用法：
#   chmod +x deploy.sh
#   ./deploy.sh            # 全新：基库 + 补丁
...
BASE_SQL="$SQL_DIR/Crmeb_v3.0.sql"
PATCH_SQL="$SCRIPT_DIR/02_patches_all.sql"
FULL_DATA="$ROOT_DIR/db-data/crmeb_full_data_export.sql"
...
if [[ "$MODE" != "patch" ]]; then
  ...
  echo "[2/3] import base Crmeb_v3.0.sql (may take a few minutes)..."
