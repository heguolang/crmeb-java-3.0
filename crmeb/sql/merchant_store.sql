...
-- 1. 门店（扩展 eb_system_store 提货点表）
ALTER TABLE `eb_system_store`
  ADD COLUMN `self_pickup` tinyint(1) NOT NULL DEFAULT 1 COMMENT '是否支持到店自提：1=是 0=否',
  ADD COLUMN `delivery` tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否支持上门配送：1=是 0=否',
  ADD COLUMN `delivery_radius` decimal(10,2) NOT NULL DEFAULT 5.00 COMMENT '配送服务半径(公里)',
  ADD COLUMN `verify_fee` decimal(10,2) NOT NULL DEFAULT 0.00 COMMENT '门店核销服务费',
  ADD COLUMN `pickup_fee` decimal(10,2) NOT NULL DEFAULT 0.00 COMMENT '到店自提服务费',
  ADD COLUMN `delivery_fee` decimal(10,2) NOT NULL DEFAULT 0.00 COMMENT '上门配送服务费',
  ADD COLUMN `leader_uid` int(11) NOT NULL DEFAULT 0 COMMENT '门店负责人用户UID(eb_user.uid)',
  ADD COLUMN `leader_name` varchar(64) DEFAULT '' COMMENT '门店负责人昵称(冗余)';

-- 2. 产品门店服务权限（单品级）
ALTER TABLE `eb_store_product`
  ADD COLUMN `is_store` tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否支持门店服务：1=是 0=否',
  ADD COLUMN `store_self_pickup` tinyint(1) NOT NULL DEFAULT 0 COMMENT '门店-是否支持自提：1=是 0=否',
  ADD COLUMN `store_delivery` tinyint(1) NOT NULL DEFAULT 0 COMMENT '门店-是否支持配送：1=是 0=否';
